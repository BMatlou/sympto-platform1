...
export class PatientMedicationsService {
  ...
  async create(
    ...
    if (existing) {
      throw new ConflictException(
        'This medication has already been added to the health passport.',
      );
    }
  ...
  async findOne(id: string) {
    const patientMedication =
      await this.prisma.patientMedication.findUnique({
        where: { id },
        include: {
          medication: true,
          healthPassport: {
  ...
  async update(id: string, dto: UpdatePatientMedicationDto) {
    ...
    if (dto.healthPassportId) {
      const passport = await this.prisma.healthPassport.findUnique({
        where: { id: dto.healthPassportId },
      });
    ...
    const duplicate = await this.prisma.patientMedication.findFirst({
      where: { id: { not: id }, healthPassportId, medicationId },
    });
  ...
  async scheduleReminder(id: string, dto: CreateMedicationReminderDto) {
    const medication = await this.findOne(id);
    const userId = medication.healthPassport.patient.person.user?.id;

    if (!userId) {
      throw new NotFoundException(
        'The patient account associated with this medication could not be found.',
      );
    }
