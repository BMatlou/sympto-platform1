...
export class ClinicCardService {
  ...
  async getForUser(userId: string) {
        ...
        healthPassport: {
          include: {
            ...
            patientDiagnoses: {
              include: {
                diagnosis: true,
                encounter: { include: { practitioner: { include: { person: true } } } },
              },
    ...
    const completedProcedures = (passport?.patientProcedures ?? []).filter((item) => !['CANCELLED'].includes(String(item.status)));
    ...
    return {
      ...
      allergies: activeAllergies.map((item) => ({
        ...
        source: item.verified || item.verifiedBy ? 'PRACTITIONER' : 'PATIENT',
        status: item.status,
      })),
      conditions: activeConditions.map((item) => ({
        ...
        source: item.diagnosedBy || item.treatmentPlan ? 'PRACTITIONER' : 'PATIENT',
        status: item.status,
      })),
      ...
      immunizations: (passport?.immunizations ?? []).map((item) => ({
        ...
        status: item.status,
        source: item.administeredBy || item.facility ? 'PRACTITIONER' : 'PATIENT',
      })),
