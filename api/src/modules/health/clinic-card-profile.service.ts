  ...
  async update(userId: string, input: UpdateClinicCardProfileInput) {
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.healthPassport.upsert({
        ...
        update: {
          bloodType: input.bloodType as any,
          rhesusFactor: input.rhesusFactor as any,
          organDonor: input.organDonor !== undefined ? input.organDonor : undefined,
      ...
      if (input.organDonor !== undefined) {
        await tx.$executeRaw(
          Prisma.sql`UPDATE "HealthPassport" SET "organDonorRecorded" = true WHERE "patientId" = ${patient.id}`,
        );
      }
