...
export class GoalsEngineService {
  ...
  async removeSourceEvents(patientId: string, source: string, sourceId: string) {
    await this.prisma.$executeRaw`
      DELETE FROM "HealthGoalMetricEvent"
      WHERE "patientId" = ${patientId} AND "source" = ${source} AND "sourceId" = ${sourceId}
    `;
  }

  async recomputeMatchingGoals(patientId: string, metricType: string, metricKey: string, now = new Date()) {
    const configs = await this.prisma.$queryRaw<GoalConfig[]>`
      SELECT c."healthGoalId", c."metricType", c."metricKey", c."frequency", c."frequencyTarget", c."guidanceText", c."aggregation", c."comparison"
      ...
      WHERE g."patientId" = ${patientId} AND g."status" = 'ACTIVE'
        AND c."metricType" = ${metricType} AND c."metricKey" = ${metricKey}
    `;
  ...
  async backfillJournalMetrics(patientId: string) {
    ...
    for (const [metricType, metricKey, column] of mappings) {
      await this.prisma.$executeRawUnsafe(`
        INSERT INTO "HealthGoalMetricEvent" ("patientId","metricType","metricKey","loggedValue","occurredAt","source","sourceId")
        SELECT "patientId", '${metricType}', '${metricKey}', ${column}, "createdAt", 'health-journal', "id"
        FROM "HealthJournal" WHERE "patientId" = '${patientId}' AND ${column} IS NOT NULL
      `);
    }
  }
  ...
  async backfillPatientProfileMetrics(patientId: string) {
    ...
    if (patient.weightKg != null) {
      await this.recordMetricEvent({ patientId, metricType: 'WEIGHT', metricKey: 'weight.kg', loggedValue: Number(patient.weightKg), source: 'patient-profile', sourceId: 'profile' });
    }
    ...
    if (patient.smokingStatus != null) {
      await this.recordMetricEvent({ patientId, metricType: 'SMOKING', metricKey: 'smoking.status', loggedValue: smokingMap[String(patient.smokingStatus)] ?? 0, source: 'patient-profile', sourceId: 'profile' });
    }
    if (patient.alcoholConsumption != null) {
      await this.recordMetricEvent({ patientId, metricType: 'ALCOHOL', metricKey: 'alcohol.frequency', loggedValue: alcoholMap[String(patient.alcoholConsumption)] ?? 0, source: 'patient-profile', sourceId: 'profile' });
    }
  }
  ...
  async snapshot(patientId: string) {
    ...
    const configs = await this.prisma.$queryRaw<GoalConfig[]>`
      SELECT c."healthGoalId", c."metricType", c."metricKey", c."frequency", c."frequencyTarget", c."guidanceText", c."aggregation", c."comparison"
      FROM "HealthGoalMetricConfig" c INNER JOIN "HealthGoal" g ON g."id" = c."healthGoalId"
      WHERE g."patientId" = ${patientId} AND g."status" = 'ACTIVE'
    `;
  ...
  private async evaluateConfigs(patientId: string, configs: GoalConfig[], now: Date) {
    ...
    for (const config of configs) {
      ...
      const value = await this.aggregateMetric(patientId, config.metricType, config.metricKey, start, now, config.aggregation);
      const target = Number(config.frequencyTarget ?? goal.targetValue ?? 0);
      if (!Number.isFinite(target)) continue;
      const evaluated = await this.evaluateValue(patientId, config, start, now, value, target);
  ...
  private async evaluateValue(patientId: string, config: GoalConfig, start: Date, end: Date, value: number, target: number) {
    ...
    if (config.comparison === 'AT_MOST') {
      return { currentValue: value, progressPercent: target <= 0 ? value <= target ? 100 : 0 : value <= target ? 100 : this.cap((target / value) * 100), achieved: value <= target };
    }
      ...
      : aggregation === 'AVERAGE'
        ...
        : aggregation === 'MIN'
          ...
          : aggregation === 'MAX'
            ? 'MAX("loggedValue")'
            : 'SUM("loggedValue")';
    const rows = await this.prisma.$queryRawUnsafe<Array<{ value: number | null }>>(`SELECT ${expression} AS value FROM "HealthGoalMetricEvent" WHERE "patientId" = '${patientId}' AND "metricType" = '${metricType}' AND "metricKey" = '${metricKey}' AND "occurredAt" BETWEEN '${start.toISOString()}' AND '${end.toISOString()}'`);
    return Number(rows[0]?.value ?? 0);
  }

  private async firstMetricValue(patientId: string, metricType: string, metricKey: string, start: Date, end: Date) {
    const rows = await this.prisma.$queryRaw<Array<{ loggedValue: number }>>`
      SELECT "loggedValue" FROM "HealthGoalMetricEvent"
