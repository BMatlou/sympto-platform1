...
if (!role) {
  throw new NotFoundException(
    `Default ${roleName} role not found.`,
  );
}
...
if (dto.userType === UserType.PRACTITIONER) {
  ...
  const existingPractitioner =
    await tx.practitioner.findUnique({
      where: {
  ...
  if (existingPractitioner) {
    throw new BadRequestException(
      'Practitioner registration number already exists.',
    );
  }
...
if (dto.userType === UserType.ADMIN) {
  ...
  const organization = await tx.organization.create({
    data: {
      ...
      organizationType:
        mapOrganizationType(dto.organizationType),
      email:
        dto.organizationEmail ?? null,
      phone:
