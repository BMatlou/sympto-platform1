...
export class OnboardingRepository {
  ...
  private async getPatientWithPassport(tx: Prisma.TransactionClient, userId: string) {
  ...
  async findByUser(userId: string) { return this.prisma.onboardingProgress.findUnique({ where: { userId } }); }
  async create(userId: string) { return this.prisma.onboardingProgress.create({ data: { userId } }); }
  ...
  async saveHealthGoals(userId: string, dto: UpdateHealthGoalsDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const goal of dto.goals) await tx.healthGoal.create({ data: { patientId: patient.id, title: goal.title, description: goal.description, category: goal.category, priority: goal.priority ?? 'MEDIUM', status: goal.status ?? 'ACTIVE', targetValue: goal.targetValue, currentValue: goal.currentValue, unit: goal.unit, targetDate: goal.targetDate ? new Date(goal.targetDate) : undefined, achievedAt: goal.achievedAt ? new Date(goal.achievedAt) : undefined } });
  ...
  async saveHealthJournalSettings(userId: string, dto: UpdateHealthJournalSettingsDto) {
    return this.prisma.$transaction(async (tx) => {
      const patient = await tx.patient.findUnique({ where: { userId } }); if (!patient) throw new NotFoundException('Patient not found.');
      await tx.healthJournalSettings.upsert({ where: { patientId: patient.id }, update: { trackSymptoms: dto.trackSymptoms, trackMood: dto.trackMood, trackSleep: dto.trackSleep, trackWater: dto.trackWater, trackNutrition: dto.trackNutrition, trackExercise: dto.trackExercise, trackMedications: dto.trackMedications, trackVitals: dto.trackVitals, remindersEnabled: dto.remindersEnabled, morningReminder: dto.morningReminder, afternoonReminder: dto.afternoonReminder, eveningReminder: dto.eveningReminder, weeklySummary: dto.weeklySummary, monthlySummary: dto.monthlySummary }, create: { patientId: patient.id, trackSymptoms: dto.trackSymptoms ?? true, trackMood: dto.trackMood ?? true, trackSleep: dto.trackSleep ?? true, trackWater: dto.trackWater ?? false, trackNutrition: dto.trackNutrition ?? false, trackExercise: dto.trackExercise ?? false, trackMedications: dto.trackMedications ?? true, trackVitals: dto.trackVitals ?? false, remindersEnabled: dto.remindersEnabled ?? true, morningReminder: dto.morningReminder, afternoonReminder: dto.afternoonReminder, eveningReminder: dto.eveningReminder, weeklySummary: dto.weeklySummary ?? true, monthlySummary: dto.monthlySummary ?? true } });
  ...
  async completeOnboarding(userId: string) {
    return this.prisma.$transaction(async (tx) => {
      const progress = await tx.onboardingProgress.findUnique({ where: { userId } }); if (!progress) throw new NotFoundException('Onboarding progress not found.');
      return tx.onboardingProgress.update({ where: { userId }, data: { status: OnboardingStatus.COMPLETED, completionPercentage: 100, currentStep: 11, completedAt: new Date() } });
    });
  }
