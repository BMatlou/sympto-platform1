...
export class HealthHomeService {
  ...
  async getHealthHome(userId: string) {
    ...
    const unreadCriticalNotifications = notifications.filter(
      (notification) =>
    ...
    const attention = [
      ...
      ...aiObservations
        .filter((observation) => observation.requiresAttention && !observation.reviewed)
        .map((observation) => ({
          type: 'AI_OBSERVATION',
          priority: 'HIGH',
          title: 'Sympto noticed something worth reviewing',
          body: observation.observation,
          actionUrl: '/health-journal',
        })),
      ...labOrders.flatMap((order) =>
        order.items.flatMap((item) =>
          item.labResults.flatMap((result) =>
            result.items
              .filter((resultItem) => resultItem.abnormal || resultItem.critical)
              .map((resultItem) => ({
    ...
    const journalSignals = {
      ...
      recentSymptoms: symptomLogs.slice(0, 5).map((log) => ({
        id: log.id,
        title: log.title,
        severity: log.overallSeverity,
        progression: log.progression,
        startedAt: log.startedAt,
        symptoms: log.symptoms.map((item) => item.symptom.name),
      })),
    ...
    return {
      ...
      healthSnapshot: {
        ...
        connectedDevices: devices.map((device) => ({
          ...
          measurementCount: device._count.measurements,
        })),
      },
      attention,
      today: {
