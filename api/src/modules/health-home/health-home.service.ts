...
export class HealthHomeService {
  ...
  async getHealthHome(userId: string, requestedPatientId?: string) {
    ...
    const journalSignals = {
      ...
      recentSymptoms: symptomLogs.slice(0, 10).map((log) => ({ id: log.id, title: log.title, severity: log.overallSeverity, progression: log.progression, startedAt: log.startedAt, endedAt: log.endedAt, symptoms: log.symptoms.map((item) => item.symptom.name) })),
