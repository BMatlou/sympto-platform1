...
export function MedicationReminderButton({ medicationId, medicationName, dosage, className = "" }: MedicationReminderButtonProps) {
  ...
  async function scheduleReminder() {
    ...
    try {
      ...
      toast.success("Medication reminder scheduled", {
        description: `${medicationName}${dosage ? ` · ${dosage}` : ""} will appear in Sympto at the selected time, with a device push notification when Push is enabled.`,
      });
  ...
  return <div className={`relative inline-flex h-9 ${className}`}>
    <button type="button" onClick={() => { setOpen(true); setSaved(false); }} className="inline-flex h-9 items-center gap-2 rounded-xl border border-[#24c1c4]/30 bg-[#24c1c4]/5 px-3 text-xs font-semibold text-[#0b2d54] hover:bg-[#24c1c4]/10">
      <Bell className="h-3.5 w-3.5" />Set reminder
    </button>
    {open && <div className="absolute right-0 top-11 z-20 w-[min(20rem,calc(100vw-3rem))] rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
      ...
      <p className="mt-3 text-[11px] leading-4 text-slate-400">Sympto will place it in your notification centre when it is due and send a device push notification when Push is enabled for Health reminders.</p>
    </div>}
  </div>;
}
