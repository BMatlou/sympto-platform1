...
function ClinicalList({ items }: { items: ClinicalSymptom[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item) => (
        <span key={item.id} title={item.description || item.name} className="rounded-full border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-medium text-[#0b2d54]">
          {item.name}
        </span>
      ))}
    </div>
...
export function MedicationAdherenceActions({ medicationId, medicationName, adherencePercentage }: MedicationAdherenceActionsProps) {
  ...
  }, [clinicalOpen, medicationId]);

  async function record(action: "TAKEN" | "SKIPPED") {
    if (saving) return;
    setSaving(action);
    try {
      const response = await api.post(`/patient-medications/${medicationId}/adherence`, { action, scheduledFor: new Date().toISOString() });
