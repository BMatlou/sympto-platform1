...
export default function HealthAccountNav() {
  ...
  return <div className="fixed right-4 top-4 z-50 flex items-center gap-2 sm:right-6 sm:top-5">
    ...
    {open && <div className="absolute right-0 top-14 w-72 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_18px_45px_rgba(11,45,84,0.16)]">
      <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Patient</p>
      {links.map(([href, label, Icon]) => <Link key={href} href={href} onClick={close} className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-[#0b2d54] hover:bg-[#24c1c4]/10"><Icon className="h-4 w-4" />{label}</Link>)}
    </div>}
  </div>;
}
