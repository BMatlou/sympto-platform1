...
function sameLocalDay(value?: string) { if (!value) return false; const date = new Date(value); return !Number.isNaN(date.getTime()) && localDayKey(date) === localDayKey(); }
function formatNumber(value: unknown, digits = 0) { if (value === null || value === undefined || value === "") return "—"; const parsed = Number(value); return Number.isFinite(parsed) ? parsed.toFixed(digits) : String(value); }
function findMeasurement(measurements: DashboardVital[], types: string[]) { return measurements.find((item) => types.includes(String(item.type ?? "").toUpperCase())); }
function formFromMeasurements(measurements: DashboardVital[]): FormState { const value = (types: string[]) => findMeasurement(measurements, types)?.value; return { weightKg: value(["WEIGHT", "BODY_WEIGHT"]) == null ? "" : String(value(["WEIGHT", "BODY_WEIGHT"])), heightCm: value(["HEIGHT", "BODY_HEIGHT"]) == null ? "" : String(value(["HEIGHT", "BODY_HEIGHT"])), systolicPressure: value(["BLOOD_PRESSURE_SYSTOLIC", "SYSTOLIC_PRESSURE"]) == null ? "" : String(value(["BLOOD_PRESSURE_SYSTOLIC", "SYSTOLIC_PRESSURE"])), diastolicPressure: value(["BLOOD_PRESSURE_DIASTOLIC", "DIASTOLIC_PRESSURE"]) == null ? "" : String(value(["BLOOD_PRESSURE_DIASTOLIC", "DIASTOLIC_PRESSURE"])), restingHeartRate: value(["HEART_RATE", "HEARTRATE"]) == null ? "" : String(value(["HEART_RATE", "HEARTRATE"])), oxygenSaturation: value(["OXYGEN_SATURATION", "OXYGENSATURATION"]) == null ? "" : String(value(["OXYGEN_SATURATION", "OXYGENSATURATION"])), bodyTemperature: value(["BODY_TEMPERATURE", "BODYTEMPERATURE"]) == null ? "" : String(value(["BODY_TEMPERATURE", "BODYTEMPERATURE"])), respiratoryRate: value(["RESPIRATORY_RATE", "RESPIRATORYRATE"]) == null ? "" : String(value(["RESPIRATORY_RATE", "RESPIRATORYRATE"])) }; }
...
export default function HealthVitalsSummary({ measurements = [] }: Props) {
  ...
  async function saveToday() {
    ...
    for (const key of Object.keys(form) as Array<keyof FormState>) { if (form[key] === "") continue; const value = Number(form[key]); if (Number.isFinite(value)) input[key] = value; }
    ...
    try {
      ...
      add("WEIGHT", input.weightKg, "kg"); add("HEIGHT", input.heightCm, "cm");
      ...
      add("HEART_RATE", input.restingHeartRate, "bpm"); add("OXYGEN_SATURATION", input.oxygenSaturation, "%"); add("BODY_TEMPERATURE", input.bodyTemperature, "°C"); add("RESPIRATORY_RATE", input.respiratoryRate, "/min");
      setTodayMeasurements(nextMeasurements); setForm(formFromMeasurements(nextMeasurements)); setMessage("Today’s measurements saved."); setOpen(false);
    } catch (error: any) { setMessage(error?.response?.data?.message || "We couldn’t save today’s measurements."); } finally { setBusy(false); }
  }
