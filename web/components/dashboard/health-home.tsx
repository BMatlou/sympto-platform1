...
import { Activity, CalendarDays, CheckCircle2, HeartPulse, LucideIcon, Pill, Target, Users, Watch, TriangleAlert } from "lucide-react";
...
export default function HealthHome() {
  ...
  return <ProtectedRoute><main className="min-h-screen bg-slate-50">
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <header className="rounded-3xl border border-[#24c1c4]/20 bg-gradient-to-br from-[#24c1c4]/10 via-white to-white p-6 sm:p-8"><p className="text-sm font-semibold tracking-wide text-[#0b2d54]">MY HEALTH</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-[#0b2d54]">{greetingText}, {patient.firstName}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Your health information, care, goals and connected-device activity — brought together automatically.</p></header>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card title="My goals" icon={Target} href="/health-goals">{goals.length === 0 ? <p className="text-sm text-slate-500">No active goals yet.</p> : <div className="space-y-3">{goals.slice(0, 3).map((goal) => { const progress = Array.isArray(goal.progress) ? goal.progress[goal.progress.length - 1] as Record<string, unknown> | undefined : undefined; const percent = Math.min(100, Math.max(0, Number(progress?.progressPercent) || 0)); return <div key={String(goal.id)}><div className="flex justify-between gap-3 text-sm"><span className="font-medium text-slate-800">{text(goal.title)}</span><span className="text-slate-500">{percent}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#24c1c4]" style={{ width: `${percent}%` }} /></div></div>; })}</div>}</Card>
