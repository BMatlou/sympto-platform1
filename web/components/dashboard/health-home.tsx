...
import { Activity, CalendarDays, CheckCircle2, ChevronRight, HeartPulse, Pill, Sparkles, Target, Users, Watch, TriangleAlert } from "lucide-react";
...
function Card({ title, icon: Icon, href, children }: { title: string; icon: typeof Activity; href?: string; children: React.ReactNode }) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        {href && <Link href={href} className="text-xs font-semibold text-[#0B5CAD]">View all</Link>}
      </div>
      {children}
    </section>
  );
    ...
    <main className="min-h-screen bg-[#F7F9FC]">
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card title="Appointments" icon={CalendarDays} href="/appointments">{today.upcomingAppointments[0] ? <div><p className="text-sm font-semibold text-slate-900">Next appointment</p><p className="mt-1 text-sm text-slate-600">{formatDate(today.upcomingAppointments[0].scheduledStart)}</p></div> : <p className="text-sm text-slate-500">Nothing scheduled yet.</p>}</Card>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card title="Your connected health" icon={Watch}>{wearables.devices.length === 0 ? <div><p className="text-sm text-slate-500">No wearable is connected yet.</p><p className="mt-2 text-xs text-slate-400">When you connect one, Sympto can use authorised measurements to support your health journey.</p></div> : <div className="space-y-3"><p className="text-sm font-medium text-slate-900">{wearables.devices.length} connected device{wearables.devices.length === 1 ? "" : "s"}</p><div className="grid grid-cols-3 gap-2">{[["Steps", steps], ["Heart", heartRate], ["Sleep", sleep]].map(([label, measurement]) => <div key={String(label)} className="rounded-xl bg-slate-50 p-2 text-center"><p className="text-[10px] font-medium text-slate-400">{String(label)}</p><p className="mt-1 text-xs font-bold text-slate-800">{measurement ? `${text((measurement as { value: unknown }).value)} ${(measurement as { unit: string }).unit}` : "—"}</p></div>)}</div></div>}</Card>
