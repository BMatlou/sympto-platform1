...
export default function HealthHome() {
  ...
  const historyCount = (data.encounters?.length ?? 0) + (data.labResults?.length ?? 0) + (data.imagingStudies?.length ?? 0) + (data.smartFiles?.length ?? 0);
  const clinicalSignalCount = conditions.length + allergies.length + immunizations.length;
  ...
  return <ProtectedRoute>
    <main className="min-h-screen bg-[#f4f9fb] text-[#14304d]">
      <div className="mx-auto max-w-[1300px] px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
        <section className="relative min-h-[325px] overflow-hidden rounded-[34px] bg-gradient-to-br from-[#08284a] via-[#0e4773] to-[#24babe] p-7 text-white shadow-[0_18px_52px_rgba(11,45,84,0.10)] sm:p-9 lg:p-10">
          ...
          <div className="relative grid items-center gap-8 lg:grid-cols-[1fr_310px] lg:gap-9">
            <div>
              ...
              <p className="mt-4 max-w-[560px] text-[14px] leading-7 text-white/75 sm:text-[15px]">Sympto brings your next action, your clinical context, and your health history together — so the right thing is easier to see.</p>
              <div className="mt-6 flex flex-wrap gap-2.5">
                <Link href="/log-symptom" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[15px] bg-white px-4 py-3 text-xs font-black text-[#0b2d54] shadow-sm transition hover:-translate-y-0.5"><HeartPulse className="h-4 w-4" />Log a symptom</Link>
                <Link href="/today" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[15px] border border-white/25 bg-white/10 px-4 py-3 text-xs font-extrabold text-white backdrop-blur-sm transition hover:bg-white/15">Open today’s plan <ArrowRight className="h-4 w-4" /></Link>
              </div>
            </div>
            <div className="mx-auto flex h-[235px] w-full max-w-[235px] items-center justify-center rounded-[28px] border border-white/20 bg-white/10 backdrop-blur-md lg:ml-auto">
              <div className="relative grid h-[145px] w-[145px] place-items-center rounded-full bg-[conic-gradient(#24c1c4_0_72%,rgba(255,255,255,0.17)_72%_100%)] shadow-[0_0_34px_rgba(36,193,196,0.25)]">
                <div className="absolute inset-[10px] rounded-full border border-white/15 bg-[#174c79]" />
                <div className="relative z-10 text-center"><b className="block text-[29px] font-black tracking-[-0.06em]">{todayActionCount}</b><span className="mt-1 block text-[10px] font-black uppercase tracking-[0.14em] text-white/70">today’s actions</span></div>
              </div>
              <div className="absolute bottom-[15px] left-0 right-0 text-center text-[11px] font-extrabold text-white/65">{clinicalSignalCount} clinical signals connected</div>
            </div>
          </div>
        </section>

        <div className="my-7 flex items-end justify-between gap-5 px-1">
          <div><p className="text-[11px] font-black uppercase tracking-[0.21em] text-[#71839a]">Connected health</p><h2 className="mt-1 text-[20px] font-black tracking-[-0.04em] text-[#0b2d54]">Your health, at a glance</h2></div>
          <span className="hidden text-[11px] font-bold text-[#71839a] sm:block">One connected health story</span>
        </div>

        <section className="grid gap-[15px] lg:grid-cols-3">
          <article className="relative min-h-[230px] overflow-hidden rounded-[27px] border border-[#e0ebef] bg-gradient-to-br from-white via-white to-[#f2fcf8] p-6 shadow-[0_5px_18px_rgba(11,45,84,0.035)] after:absolute after:-bottom-[68px] after:-right-[58px] after:h-[145px] after:w-[145px] after:rounded-full after:bg-[#168660]/10 after:content-['']">
            <div className="relative z-10">
              <div className="flex items-start justify-between gap-3"><div className="grid h-11 w-11 place-items-center rounded-[15px] bg-[#e8f8f1] text-[#168660]"><CheckCircle2 className="h-5 w-5" /></div><span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#71839a]">Today</span></div>
              <h3 className="mt-5 text-[19px] font-black tracking-[-0.04em] text-[#0b2d54]">What do I do today?</h3>
              <p className="mt-1.5 max-w-[260px] text-xs leading-5 text-[#71839a]">Medicines, visits, care goals, and proactive next steps in one calm view.</p>
              <div className="mt-5 grid gap-2 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
                <LargeAction href="/medications" icon={<Pill className="h-4 w-4" />} label={`${medications.length} medicine${medications.length === 1 ? "" : "s"}`} />
                <LargeAction href="/appointments" icon={<CalendarDays className="h-4 w-4" />} label={`${appointments.length} visit${appointments.length === 1 ? "" : "s"}`} />
                <LargeAction href="/health-goals" icon={<CheckCircle2 className="h-4 w-4" />} label={`${activeGoals} goal${activeGoals === 1 ? "" : "s"}`} />
              </div>
              <Link href="/today" className="mt-4 flex items-center justify-between text-xs font-black text-[#0b2d54]"><span>Open Today</span><span className="text-[#71839a]">{todayActionCount} actions <ArrowRight className="ml-1 inline h-3.5 w-3.5" /></span></Link>
            </div>
          </article>

          <article className="relative min-h-[230px] overflow-hidden rounded-[27px] border border-[#e0ebef] bg-gradient-to-br from-white via-white to-[#fff5f4] p-6 shadow-[0_5px_18px_rgba(11,45,84,0.035)] after:absolute after:-bottom-[68px] after:-right-[58px] after:h-[145px] after:w-[145px] after:rounded-full after:bg-[#c94d51]/10 after:content-['']">
            <div className="relative z-10">
              <Link href="/health-passport" className="block"><div className="flex items-start justify-between gap-3"><div className="grid h-11 w-11 place-items-center rounded-[15px] bg-[#fff0ef] text-[#c94d51]"><ShieldCheck className="h-5 w-5" /></div><span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#71839a]">Clinic</span></div><h3 className="mt-5 text-[19px] font-black tracking-[-0.04em] text-[#0b2d54]">My Clinic Card</h3><p className="mt-1.5 max-w-[270px] text-xs leading-5 text-[#71839a]">Essential clinical context, vital signs, and secure health information.</p></Link>
              <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2"><div className="rounded-2xl bg-white p-3.5 ring-1 ring-[#e0ebef]"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-[#71839a]"><HeartPulse className="h-4 w-4 text-[#24c1c4]" />Blood pressure</div><p className="mt-2 text-xl font-black text-[#0b2d54]">{bpValue}</p></div><div className="rounded-2xl bg-white p-3.5 ring-1 ring-[#e0ebef]"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-[#71839a]"><Watch className="h-4 w-4 text-[#24c1c4]" />Watch heart</div><p className="mt-2 text-xl font-black text-[#0b2d54]">{heartValue}</p></div></div>
              <div className="mt-3 grid gap-2 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3"><CountPill icon={<ShieldCheck className="h-4 w-4 text-[#24c1c4]" />} count={conditions.length} label="conditions" /><CountPill icon={<TriangleAlert className="h-4 w-4 text-[#24c1c4]" />} count={allergies.length} label="allergies" /><CountPill icon={<CheckCircle2 className="h-4 w-4 text-[#24c1c4]" />} count={immunizations.length} label="vaccines" /></div>
            </div>
          </article>

          <article className="relative min-h-[230px] overflow-hidden rounded-[27px] border border-[#e0ebef] bg-gradient-to-br from-white via-white to-[#f2f7ff] p-6 shadow-[0_5px_18px_rgba(11,45,84,0.035)] after:absolute after:-bottom-[68px] after:-right-[58px] after:h-[145px] after:w-[145px] after:rounded-full after:bg-[#3f75bd]/10 after:content-['']">
            <div className="relative z-10">
              <Link href="/health-journal" className="block"><div className="flex items-start justify-between gap-3"><div className="grid h-11 w-11 place-items-center rounded-[15px] bg-[#edf4ff] text-[#3f75bd]"><FolderOpen className="h-5 w-5" /></div><span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#71839a]">History</span></div><h3 className="mt-5 text-[19px] font-black tracking-[-0.04em] text-[#0b2d54]">My History &amp; Papers</h3><p className="mt-1.5 max-w-[270px] text-xs leading-5 text-[#71839a]">Episodes, visits, results, scans, and documents arranged as one story.</p><div className="mt-5 flex items-center justify-between rounded-2xl bg-white p-3.5 ring-1 ring-[#e0ebef]"><div><p className="text-sm font-black text-[#0b2d54]">Open health folder</p><p className="mt-1 text-[10px] font-semibold text-[#71839a]">Episodes · Encounters · Results · Imaging · Papers</p></div><ArrowRight className="h-5 w-5 shrink-0 text-[#3f75bd]" /></div></Link>
              <div className="mt-4 flex items-center justify-between text-xs font-black text-[#0b2d54]"><span>{historyCount} connected records</span><span className="text-[#71839a]">View history <ArrowRight className="ml-1 inline h-3.5 w-3.5" /></span></div>
            </div>
          </article>
        </section>

        <div className="my-7 flex items-end justify-between gap-5 px-1"><div><h2 className="text-[19px] font-black tracking-[-0.04em] text-[#0b2d54]">Your health story</h2></div><span className="text-[11px] font-bold text-[#71839a]">Connected context</span></div>

        <section className="grid gap-[15px] lg:grid-cols-[1.18fr_0.82fr]">
          <article className="rounded-[27px] border border-[#e0ebef] bg-white p-6 shadow-[0_5px_18px_rgba(11,45,84,0.035)]">
            <div className="flex items-center justify-between gap-4"><h3 className="text-[17px] font-black tracking-[-0.03em] text-[#0b2d54]">Next on your timeline</h3><Link href="/health-journal" className="text-[11px] font-black text-[#24c1c4]">View full history <ArrowRight className="ml-1 inline h-3.5 w-3.5" /></Link></div>
            <div className="mt-3">
              <div className="grid grid-cols-[36px_1fr_auto] items-center gap-3 border-b border-[#edf2f4] py-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf7f8] text-[#0b2d54]"><CalendarDays className="h-4 w-4" /></div><div><b className="block text-xs text-[#0b2d54]">{appointments[0] ? "Upcoming appointment" : "Your care plan"}</b><p className="mt-1 text-[11px] text-[#71839a]">{appointments[0] ? display((appointments[0] as Record<string, unknown>).scheduledStart ?? (appointments[0] as Record<string, unknown>).startTime ?? "Appointment scheduled") : activeGoals > 0 ? `${activeGoals} care goal${activeGoals === 1 ? "" : "s"} still in progress.` : "No upcoming action is currently due."}</p></div><span className="text-[10px] font-extrabold text-[#71839a]">Soon</span></div>
              <div className="grid grid-cols-[36px_1fr_auto] items-center gap-3 border-b border-[#edf2f4] py-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf7f8] text-[#0b2d54]"><HeartPulse className="h-4 w-4" /></div><div><b className="block text-xs text-[#0b2d54]">Health signals</b><p className="mt-1 text-[11px] text-[#71839a]">{conditions.length} condition{conditions.length === 1 ? "" : "s"}, {allergies.length} allerg{allergies.length === 1 ? "y" : "ies"}, and {medications.length} active medicine{medications.length === 1 ? "" : "s"} connected.</p></div><span className="text-[10px] font-extrabold text-[#71839a]">Now</span></div>
              <div className="grid grid-cols-[36px_1fr_auto] items-center gap-3 py-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf7f8] text-[#0b2d54]"><FolderOpen className="h-4 w-4" /></div><div><b className="block text-xs text-[#0b2d54]">Health record</b><p className="mt-1 text-[11px] text-[#71839a]">{historyCount > 0 ? `${historyCount} connected record${historyCount === 1 ? "" : "s"} available in your health history.` : "Your health history is ready for new records."}</p></div><span className="text-[10px] font-extrabold text-[#71839a]">History</span></div>
            </div>
          </article>

          <article className="relative overflow-hidden rounded-[27px] bg-[#0b2d54] p-6 text-white shadow-[0_5px_18px_rgba(11,45,84,0.035)] after:absolute after:-bottom-[110px] after:-right-[100px] after:h-[210px] after:w-[210px] after:rounded-full after:bg-[#24c1c4]/20 after:content-['']">
            <div className="relative z-10"><div className="flex items-center justify-between gap-4"><h3 className="text-[17px] font-black tracking-[-0.03em]">Clinical context</h3><Link href="/health-passport" className="text-[11px] font-black text-[#24c1c4]">View details <ArrowRight className="ml-1 inline h-3.5 w-3.5" /></Link></div><p className="mt-2 text-xs leading-5 text-white/65">The signals Sympto is connecting to make your next steps more relevant.</p><div className="mt-5 grid gap-2"><div className="flex items-center justify-between gap-3 rounded-[15px] border border-white/10 bg-white/10 px-3.5 py-3 text-[11px]"><span className="text-white/65">Active medicines</span><b>{medications.length}</b></div><div className="flex items-center justify-between gap-3 rounded-[15px] border border-white/10 bg-white/10 px-3.5 py-3 text-[11px]"><span className="text-white/65">Active conditions</span><b>{conditions.length}</b></div><div className="flex items-center justify-between gap-3 rounded-[15px] border border-white/10 bg-white/10 px-3.5 py-3 text-[11px]"><span className="text-white/65">Recorded allergies</span><b>{allergies.length}</b></div><div className="flex items-center justify-between gap-3 rounded-[15px] border border-white/10 bg-white/10 px-3.5 py-3 text-[11px]"><span className="text-white/65">Upcoming visits</span><b>{appointments.length}</b></div></div><div className="mt-5 flex items-center justify-between gap-3 text-[11px] text-white/60"><span>{watchConnected ? "Smart Watch connected" : "Manual vitals remain available"}</span><strong className="text-[#24c1c4]">✦ Protected</strong></div></div>
          </article>
        </section>

        <section className="mt-[15px] overflow-hidden rounded-[27px] border border-[#e0ebef] bg-white p-5 shadow-[0_5px_18px_rgba(11,45,84,0.035)] sm:p-6">
          <div className="flex items-center justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#24c1c4]">Vitals &amp; connected health</p><p className="mt-1 text-xs font-semibold text-[#71839a]">Your existing weight, height and BMI experience stays here.</p></div><Link href="/wearables" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#0b2d54] px-4 py-2 text-[11px] font-black text-white">{watchConnected ? "View watch" : "Connect watch"}<ArrowRight className="h-3.5 w-3.5" /></Link></div>
          <WeightBodySizeCard weightKg={data.healthSnapshot?.weightKg} heightCm={data.healthSnapshot?.heightCm} bmi={data.healthSnapshot?.bmi} patientId={patientId} reload={reload} />
        </section>
      </div>
    </main>
  </ProtectedRoute>;
