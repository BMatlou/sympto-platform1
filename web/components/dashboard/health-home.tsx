...
function Card({ title, icon: Icon, href, children }: { title: string; icon: LucideIcon; href?: string; children: React.ReactNode }) {
  const content = <section className={`rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition ${href ? "hover:-translate-y-0.5 hover:border-[#24c1c4]/40 hover:shadow-md focus-within:ring-2 focus-within:ring-[#24c1c4]/30" : ""}`}><div className="mb-4 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#24c1c4]/10 text-[#0b2d54]"><Icon className="h-4 w-4" /></span><h2 className="font-semibold text-[#0b2d54]">{title}</h2></div>{href && <span className="text-xs font-semibold text-[#0b2d54]">View all</span>}</div>{children}</section>;
...
export default function HealthHome() {
  ...
  return <ProtectedRoute><main className="min-h-screen bg-slate-50">
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <header className="rounded-3xl border border-[#24c1c4]/20 bg-gradient-to-br from-[#24c1c4]/10 via-white to-white p-6 sm:p-8"><p className="text-sm font-semibold tracking-wide text-[#0b2d54]">MY HEALTH</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-[#0b2d54]">{greetingText}, {firstName}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Your health information, care, goals and connected-device activity — brought together automatically.</p></header>
      {attention.length > 0 && <Card title="Needs your attention" icon={TriangleAlert}><div className="space-y-3">{attention.slice(0, 3).map((item, index) => <div key={`${item.type}-${index}`} className="rounded-xl border border-amber-100 bg-amber-50 p-3"><p className="text-sm font-semibold text-[#0b2d54]">{item.title}</p><p className="mt-1 text-xs leading-5 text-slate-600">{item.description}</p>{item.actionUrl && <Link href={item.actionUrl} className="mt-2 inline-block text-xs font-semibold text-[#0b2d54] underline underline-offset-2">{item.actionLabel || "View"}</Link>}</div>)}</div></Card>}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <button type="button" onClick={openWeightModal} className="rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#24c1c4]/40 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#24c1c4]/30"><div className="mb-4 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#24c1c4]/10 text-[#0b2d54]"><Scale className="h-4 w-4" /></span><h2 className="font-semibold text-[#0b2d54]">BMI</h2></div><span className="text-xs font-semibold text-[#0b2d54]">Update</span></div><p className="text-3xl font-bold text-[#0b2d54]">{bmi != null ? bmi.toFixed(1) : "—"}</p><p className="mt-1 text-sm text-slate-500">{bmiCategory || "Add your weight and height"}</p><p className="mt-3 text-xs text-slate-400">Weight: {healthSnapshot.weightKg != null ? `${healthSnapshot.weightKg} kg` : "—"}</p></button>
        <Card title="Medication" icon={Pill} href="/medications"><p className="text-3xl font-bold text-[#0b2d54]">{today.activeMedications.length}</p><p className="mt-1 text-sm text-slate-500">active medication{today.activeMedications.length === 1 ? "" : "s"}</p>{medicationNotifications.length > 0 && <p className="mt-3 rounded-lg bg-[#24c1c4]/10 px-3 py-2 text-xs font-semibold text-[#0b2d54]">{medicationNotifications.length} medication notification{medicationNotifications.length === 1 ? "" : "s"} to review</p>}</Card>
        <Card title="Appointments" icon={CalendarDays} href="/appointments">{nextAppointment ? <div><p className="text-sm font-semibold text-[#0b2d54]">{text(nextAppointment.title || nextAppointment.type || "Appointment")}</p><p className="mt-1 text-sm text-slate-600">{formatDate(nextAppointment.scheduledStart)}</p></div> : <p className="text-sm text-slate-500">Nothing scheduled yet.</p>}</Card>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card title="Your connected health" icon={Watch}>{wearables.devices.length === 0 ? <p className="text-sm text-slate-500">No wearable is connected yet.</p> : <div className="space-y-3"><p className="text-sm font-medium text-[#0b2d54]">{wearables.devices.length} connected device{wearables.devices.length === 1 ? "" : "s"}</p><div className="grid grid-cols-3 gap-2">{wearableMetric("Steps", findMeasurement("STEPS"))}{wearableMetric("Heart", findMeasurement("HEART_RATE"))}{wearableMetric("Sleep", findMeasurement("SLEEP"))}</div></div>}</Card>
