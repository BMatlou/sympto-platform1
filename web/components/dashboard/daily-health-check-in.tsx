    ...
    <div className={embedded ? "" : "rounded-[27px] border border-[#dfebef] bg-white p-0 shadow-[0_16px_42px_rgba(11,45,84,.06)]"}>
      ...
      <div className={embedded ? "grid gap-[15px] lg:grid-cols-[1.05fr_.95fr]" : "grid gap-[15px] p-4 lg:grid-cols-[1.05fr_.95fr]"}>
        <article className="rounded-[27px] border border-[#dfebef] bg-white p-5 shadow-[0_6px_20px_rgba(11,45,84,.035)] sm:p-[23px] lg:row-span-2">
          ...
          <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
            {moodOptions.map((option) => {
              ...
              const iconTone = {
                "very-low": "bg-[#f1eefe] text-[#7356b2]",
                low: "bg-[#edf4ff] text-[#3f75bd]",
                okay: "bg-[#eef8f4] text-[#168660]",
                good: "bg-[#e5f7f6] text-[#148e92]",
          ...
          <div className="relative mt-2.5 h-2.5 overflow-visible rounded-full bg-[#dfeef1]">
            <div className="absolute inset-y-0 left-0 w-[70%] rounded-full bg-[#83d6c0]" /><div className="absolute inset-y-0 left-[70%] w-[14%] bg-[#f4d993]" /><div className="absolute inset-y-0 left-[84%] right-0 rounded-r-full bg-[#f09a92]" />
            <input aria-label="Stress level" type="range" min={1} max={10} value={stressLevel} onChange={(event) => { setStressLevel(Number(event.target.value)); setStressTouched(true); }} className="absolute inset-0 h-2.5 w-full cursor-pointer opacity-0" />
            <span className="pointer-events-none absolute top-1/2 h-[19px] w-[19px] rounded-full border-4 border-[#0b2d54] bg-white shadow-[0_2px_6px_rgba(11,45,84,.2)] transition-all" style={{ left: `${((stressLevel - 1) / 9) * 100}%`, transform: "translate(-50%,-50%)" }} />
          </div>
          ...
          <div className="mt-5 flex flex-col justify-between gap-3 border-t border-[#eef3f4] pt-4 sm:flex-row sm:items-center">
            <div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#f2efff] text-[#7356b2]"><Moon className="h-4 w-4" /></span><div><b className="block text-[18px] font-black tracking-[-.05em] text-[#0b2d54]">{sleepHours}h</b><span className="text-[11px] text-[#74859a]">Rest and recovery</span></div></div>
            <div className="flex flex-wrap gap-1.5">{sleepOptions.map((option) => <button key={option.value} type="button" onClick={() => setSleepQuality(option.value)} className={`rounded-[11px] border px-2 py-1.5 text-[9px] font-black ${sleepQuality === option.value ? "border-[#b9e6e6] bg-[#eef7f8] text-[#0b2d54]" : "border-[#dfebef] bg-white text-[#74859a]"}`}>{option.label}</button>)}</div>
          </div>

          <div className="mt-3 flex flex-col justify-between gap-3 border-t border-[#eef3f4] pt-4 sm:flex-row sm:items-center">
            <div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf4ff] text-[#3f75bd]"><Dumbbell className="h-4 w-4" /></span><div><b className="block text-[18px] font-black tracking-[-.05em] text-[#0b2d54]">{exerciseMinutes} min</b><span className="text-[11px] text-[#74859a]">Exercise today</span></div></div>
            <div className="flex flex-wrap gap-1.5">{exerciseOptions.slice(0, 5).map((value) => <button key={value} type="button" onClick={() => setExerciseMinutes(value)} className={`rounded-[11px] border px-2.5 py-1.5 text-[9px] font-black ${exerciseMinutes === value ? "border-[#b9e6e6] bg-[#eef7f8] text-[#0b2d54]" : "border-[#dfebef] bg-white text-[#74859a]"}`}>{value === 0 ? "None" : value}</button>)}</div>
          </div>
          <div className="mt-4 flex justify-end"><div className="flex items-center gap-2"><button type="button" aria-label="Decrease sleep hours" onClick={() => setSleepHours((value) => Math.max(0, Number((value - .5).toFixed(1))))} className="grid h-8 w-8 place-items-center rounded-lg bg-[#f5f8fb] text-[#0b2d54]"><Minus className="h-3.5 w-3.5" /></button><span className="min-w-9 text-center text-xs font-black text-[#0b2d54]">{sleepHours}h</span><button type="button" aria-label="Increase sleep hours" onClick={() => setSleepHours((value) => Math.min(24, Number((value + .5).toFixed(1))))} className="grid h-8 w-8 place-items-center rounded-lg bg-[#f5f8fb] text-[#0b2d54]"><Plus className="h-3.5 w-3.5" /></button></div></div>
        </article>
        ...
        <article className="rounded-[27px] border border-[#dfebef] bg-white p-5 shadow-[0_6px_20px_rgba(11,45,84,.035)] sm:p-[23px]">
          ...
          <div className="mt-4 flex items-center gap-4">
            <div className="relative h-[116px] w-[82px] overflow-hidden rounded-[7px] rounded-b-[19px] border-[3px] border-[#acd8e3] bg-gradient-to-br from-white/90 to-[#e0f7fa]/40 shadow-[inset_8px_0_13px_rgba(255,255,255,.58),inset_-8px_0_13px_rgba(55,132,164,.08),0_8px_20px_rgba(39,133,167,.12)]">
              ...
              <div className="absolute bottom-0 left-0 right-0 transition-all duration-500" style={{ height: `${Math.max(3, waterPercent)}%`, background: "linear-gradient(180deg,#74d8dc,#24aeb9)", boxShadow: "inset 0 5px 9px rgba(255,255,255,.28)" }}>
                ...
                <span className="absolute bottom-5 left-[18px] h-1.5 w-1.5 animate-pulse rounded-full bg-white/70" /><span className="absolute bottom-9 left-[53px] h-1 w-1 animate-pulse rounded-full bg-white/70" /><span className="absolute bottom-3.5 left-[34px] h-[3px] w-[3px] animate-pulse rounded-full bg-white/70" />
              </div>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">{waterOptions.map((amount) => <button key={amount} type="button" onClick={() => setWaterIntakeMl((value) => value + amount)} className="rounded-[10px] border border-[#dfebef] bg-white px-2.5 py-1.5 text-[10px] font-black text-[#0b2d54] hover:bg-[#f7fcfc]">+{amount} ml</button>)}<button type="button" onClick={() => setWaterIntakeMl(0)} className="rounded-[10px] border-0 bg-transparent px-1 py-1.5 text-[10px] font-extrabold text-[#74859a]">Reset</button></div>
        </article>
        ...
        <article className="rounded-[27px] border border-[#dfebef] bg-white p-5 shadow-[0_6px_20px_rgba(11,45,84,.035)] sm:p-[23px]">
          ...
          <div className="mt-4 rounded-[21px] border border-[#dcecf0] bg-gradient-to-br from-[#f1f7ff] to-[#eefbfa] p-4">
            <div className="flex items-center gap-4"><div className="grid h-[88px] w-[88px] shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(#24c1c4 0 ${movementProgress}%,#dfeef1 ${movementProgress}% 100%)`, boxShadow: "0 7px 16px rgba(36,193,196,.14)" }}><div className="grid h-[68px] w-[68px] place-items-center rounded-full bg-[#f7fcfc] text-center"><b className="block text-[21px] font-black tracking-[-.06em] text-[#0b2d54]">{exerciseMinutes}</b><span className="text-[10px] text-[#74859a]">minutes</span></div></div><div><b className="block text-sm text-[#0b2d54]">{movementTitle[0]}</b><span className="mt-1 block text-[11px] leading-5 text-[#74859a]">{movementTitle[1]}</span></div></div>
            <div className="mt-4 flex items-center px-1">{Array.from({ length: 5 }).map((_, index) => { const active = movementProgress >= (index + 1) * 22.5; return <span key={`trail-${index}`} className="flex flex-1 items-center last:flex-none">{index > 0 && <span className={`h-[3px] flex-1 ${active ? "bg-gradient-to-r from-[#24c1c4] to-[#9be5e0]" : "bg-[#d9e9ed]"}`} />}<span className={`relative z-10 h-2.5 w-2.5 shrink-0 rounded-full border-[3px] border-[#f2f9fa] shadow-[0_0_0_1px_#cfe3e7] ${active ? "bg-[#24c1c4] shadow-[0_0_0_1px_#24c1c4,0_3px_8px_rgba(36,193,196,.25)]" : "bg-[#d9e9ed]"}`} /></span>; })}</div>
            <div className="mt-3 flex flex-wrap gap-1.5">{exerciseOptions.map((value) => <button key={value} type="button" onClick={() => setExerciseMinutes(value)} className={`rounded-[10px] border px-2.5 py-1.5 text-[10px] font-black ${exerciseMinutes === value ? "border-[#0b2d54] bg-[#0b2d54] text-white" : "border-[#d6e6ea] bg-white text-[#74859a]"}`}>{value === 0 ? "None" : `${value} min`}</button>)}</div>
          </div>
      ...
      {(message || error || hasInput) && (
        <div className={embedded ? "mt-4 border-t border-[#edf2f5] pt-4" : "border-t border-[#edf2f5] px-5 py-5 sm:px-6"}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-[10px] leading-4 text-[#9aa8b7]">Your answers become structured Health Journal data.</p><button type="button" disabled={saving || !hasInput} onClick={save} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[13px] bg-[#0b2d54] px-4 py-2.5 text-xs font-black text-white shadow-[0_8px_22px_rgba(11,45,84,.16)] disabled:cursor-not-allowed disabled:opacity-45"><Save className="h-4 w-4" />{saving ? "Saving…" : savedJournal ? "Update check-in" : "Save check-in"}</button></div>
        </div>
      )}
    </div>
  );
