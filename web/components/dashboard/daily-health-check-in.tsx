    ...
    <div className={surface}>
      ...
      <div className={embedded ? "grid gap-[15px] lg:grid-cols-[1.05fr_.95fr]" : "grid gap-[15px] p-4 lg:grid-cols-[1.05fr_.95fr]"}>
        <article className="rounded-[27px] border border-[#dfebef] bg-white p-5 shadow-[0_6px_20px_rgba(11,45,84,.035)] sm:p-[23px] lg:row-span-2">
          ...
          <div className="mt-5 border-t border-[#eef3f4] pt-4">
            <div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf4ff] text-[#3f75bd]"><Dumbbell className="h-4 w-4" /></span><div><b className="block text-[18px] font-black tracking-[-.05em] text-[#0b2d54]">{exerciseMinutes} min</b><span className="text-[11px] text-[#74859a]">Exercise today</span></div></div><span className="text-xs font-black text-[#3f75bd]">{movementProgress}%</span></div>
            {exerciseGoal && <p className="mt-2 flex items-center gap-1.5 text-[10px] font-semibold text-[#3f75bd]"><Target className="h-3 w-3" />{exerciseGoal.title ?? "Exercise goal"} · {exerciseGoalMinutes} min target</p>}
            <div className="mt-3 flex flex-wrap gap-1.5">{exerciseOptions.map((value) => <button key={value} type="button" onClick={() => { setExerciseMinutes(value); void syncGoal(exerciseGoal, "EXERCISE", value); }} className={`rounded-[10px] border px-2.5 py-1.5 text-[9px] font-black ${exerciseMinutes === value ? "border-[#0b2d54] bg-[#0b2d54] text-white" : "border-[#d6e6ea] bg-white text-[#74859a]"}`}>{value === 0 ? "None" : value}</button>)}</div>
          </div>
        </article>
        ...
        <article className="rounded-[27px] border border-[#dfebef] bg-white p-5 shadow-[0_6px_20px_rgba(11,45,84,.035)] sm:p-[23px]">
          <div className="flex items-start justify-between gap-4"><div><h3 className="text-[17px] font-black text-[#0b2d54]">Movement</h3><p className="mt-1 text-xs text-[#74859a]">Make progress feel visible</p></div><span className="rounded-[11px] bg-[#e9f8f1] px-2.5 py-2 text-[10px] font-black text-[#168660]">Today</span></div>
          ...
          <p className="mt-3 text-[11px] leading-[1.55] text-[#74859a]">Choose the time that matches your day. Sympto turns it into measurable progress against your movement goal.</p>
        </article>
      </div>
      ...
      <div className={`${embedded ? "mt-4" : "mt-4 px-4 pb-4"} flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between`}>
        <p className="max-w-xl text-[10px] leading-4 text-[#9aa8b7]">Mood, sleep, stress, hydration and movement are stored in your Health Journal. Goal-linked measures also update their progress history.</p>
