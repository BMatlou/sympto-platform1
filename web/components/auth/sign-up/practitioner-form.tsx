...
import { BadgeCheck, Lock, Mail, Stethoscope, User } from "lucide-react";
...
import { MEDICAL_AUTHORITIES } from "@/data/medical-authorities";
import { MEDICAL_PROFESSIONS } from "@/data/medical-professions";
...
function getPasswordStrength(password: string) {
  ...
  if (score <= 4) {
    return { label: "Medium", color: "text-amber-500", score };
  }

  return { label: "Strong", color: "text-emerald-500", score };
}

export default function PractitionerForm(props: PractitionerFormProps) {
  ...
  const authority = useWatch({ control, name: "medicalAuthority" });
  ...
  const cityOptions = useMemo(
    () => (country && province ? getCities(country, province) : []),
    [country, province]
  );

  const authorities = useMemo(
    () => (country && MEDICAL_AUTHORITIES[country] ? MEDICAL_AUTHORITIES[country] : []),
    [country]
  );

  const professions = useMemo(
    () => (authority && MEDICAL_PROFESSIONS[authority] ? MEDICAL_PROFESSIONS[authority] : []),
    [authority]
  );
    ...
    <div className="space-y-5">
        ...
        <Controller
          ...
          render={({ field }) => (
            <PhoneInput
              ...
              numberInputProps={{
                className:
                  "w-full rounded-xl border border-slate-300 py-3 pl-4 pr-4 outline-none transition focus:border-[#24C1C4] focus:ring-2 focus:ring-[#24C1C4]/20",
              }}
            />
          )}
        />
      ...
      <div>
        ...
        <select
          {...register("province")}
          disabled={!country}
          className="w-full rounded-xl border border-slate-300 py-3 px-4 outline-none transition disabled:bg-slate-100 disabled:text-slate-400 focus:border-[#24C1C4] focus:ring-2 focus:ring-[#24C1C4]/20"
        >
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-700">Medical Council / Registration Authority</label>
        <select
          {...register("medicalAuthority")}
          disabled={!country}
          className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition disabled:bg-slate-100 disabled:text-slate-400 focus:border-[#24C1C4] focus:ring-2 focus:ring-[#24C1C4]/20"
        >
          <option value="">{country ? "Select authority" : "Select country first"}</option>
        ...
        {errors.medicalAuthority && <p className="mt-1 text-sm text-red-500">{errors.medicalAuthority.message}</p>}
      </div>
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-700">Registration / License Number</label>
        <div className="relative">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <select
            {...register("profession")}
            disabled={!authority}
            className="w-full rounded-xl border border-slate-300 py-3 pl-12 pr-4 outline-none transition disabled:bg-slate-100 disabled:text-slate-400 focus:border-[#24C1C4] focus:ring-2 focus:ring-[#24C1C4]/20"
          >
            <option value="">{authority ? "Select profession" : "Select medical authority first"}</option>
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-700">Practice Name (Optional)</label>
        <input
          {...register("practiceName")}
          type="text"
          placeholder="Healthy Life Medical Centre"
          className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-[#24C1C4] focus:ring-2 focus:ring-[#24C1C4]/20"
        />
      </div>
      <div>
