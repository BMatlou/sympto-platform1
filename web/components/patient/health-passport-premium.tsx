...
export default function HealthPassportPremium() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      organDonor: card.emergency?.organDonorRecorded ? (card.emergency?.organDonor ? "true" : "false") : "",
  ...
  const save = async () => {
    ...
    try {
      ...
      setEditing(false); setMessage("Your Clinic Card has been updated.");
  ...
  const donorLabel = computed?.emergency?.organDonorRecorded ? (computed.emergency.organDonor ? "Yes" : "No") : "Not recorded";
  ...
  return <ProtectedRoute>
    <main className="min-h-screen bg-[#f5f8fb] pb-10 text-slate-800"><div className="mx-auto max-w-5xl px-4 py-5 sm:px-6 sm:py-7">
      ...
      <section className="relative overflow-hidden rounded-[32px] bg-gradient-to-br from-rose-700 via-rose-600 to-[#a61b42] p-6 text-white shadow-[0_24px_60px_rgba(159,32,67,0.24)] sm:p-8"><div className="absolute -right-16 -top-20 h-48 w-48 rounded-full bg-white/10 blur-2xl" /><div className="absolute -bottom-24 -left-12 h-56 w-56 rounded-full bg-black/10 blur-2xl" /><div className="relative">
        ...
        <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Age", age(computed.patient?.dateOfBirth)], ["Gender", human(computed.patient?.gender)], ["Blood group", human(computed.emergency?.bloodType)], ["Rhesus", human(computed.emergency?.rhesusFactor)]].map(([name, value]) => <div key={name} className="rounded-2xl bg-white/10 p-3.5 ring-1 ring-white/10"><p className="text-[10px] font-semibold uppercase tracking-wide text-white/55">{name}</p><p className="mt-1.5 text-sm font-bold text-white">{value}</p></div>)}</div>
      ...
      {editing && <section className="mt-5 rounded-[26px] border border-rose-100 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start justify-between"><div><h2 className="text-lg font-bold text-[#0b2d54]">Update your Clinic Card</h2><p className="mt-1 text-sm text-slate-500">Only your own patient information is editable here.</p></div><button onClick={() => setEditing(false)} className="rounded-xl p-2 text-slate-400"><X className="h-5 w-5" /></button></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-500">Blood type</span><select value={form.bloodType} onChange={(e) => setForm((v) => ({ ...v, bloodType: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm"><option value="">Not recorded</option>{["A_POSITIVE","A_NEGATIVE","B_POSITIVE","B_NEGATIVE","AB_POSITIVE","AB_NEGATIVE","O_POSITIVE","O_NEGATIVE","UNKNOWN"].map((v) => <option key={v} value={v}>{human(v)}</option>)}</select></label>
          <label className="block"><span className="mb-1.5 block text-xs font-semibold text-slate-500">Rhesus factor</span><select value={form.rhesusFactor} onChange={(e) => setForm((v) => ({ ...v, rhesusFactor: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm"><option value="">Not recorded</option><option value="POSITIVE">Positive</option><option value="NEGATIVE">Negative</option><option value="UNKNOWN">Unknown</option></select></label>
          ...
          <label className="sm:col-span-2 block"><span className="mb-1.5 block text-xs font-semibold text-slate-500">Emergency notes</span><textarea rows={3} value={form.emergencyNotes} onChange={(e) => setForm((v) => ({ ...v, emergencyNotes: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm" /></label>
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-2">
        <Panel title="Critical & emergency information" icon={<ShieldCheck className="h-5 w-5" />}><div className="rounded-2xl bg-rose-50 p-4"><p className="text-[10px] font-bold uppercase text-rose-600">Organ donor</p><p className="mt-1 font-bold text-[#0b2d54]">{donorLabel}</p></div>{computed.emergency?.emergencyNotes && <div className="mt-3 rounded-2xl bg-amber-50 p-4 text-sm text-amber-900"><span className="font-bold">Emergency notes:</span> {computed.emergency.emergencyNotes}</div>}</Panel>
