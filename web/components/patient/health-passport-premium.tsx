...
export default function HealthPassportPremium() {
  ...
  const loadCard = async () => {
    ...
    try {
      ...
      setCardError(error?.response?.data?.message || "We couldn't load your Clinic Card.");
    } finally {
    ...
    <main className="min-h-screen bg-[#f5f8fb] pb-10 text-slate-800">
      <div className="mx-auto max-w-5xl px-4 py-5 sm:px-6 sm:py-7">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <Panel title="Critical & emergency information" icon={<ShieldCheck className="h-5 w-5" />}><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl bg-rose-50 p-4"><p className="text-[10px] font-bold uppercase text-rose-600">Organ donor</p><p className="mt-1 font-bold text-[#0b2d54]">{computed.emergency?.organDonor ? "Yes" : "No"}</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase text-slate-500">Emergency contact</p><p className="mt-1 font-bold text-[#0b2d54]">{text(computed.emergency?.contacts?.[0]?.fullName)}</p><p className="mt-1 text-xs text-slate-500">{text(computed.emergency?.contacts?.[0]?.phoneNumber)}</p></div></div>{computed.emergency?.emergencyNotes && <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="text-[10px] font-bold uppercase text-amber-700">Emergency notes</p><p className="mt-1 text-sm leading-6 text-amber-950">{computed.emergency.emergencyNotes}</p></div>}<Link href="/emergency-contacts" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">Manage emergency contacts <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>

          <Panel title="Current measurements" icon={<Activity className="h-5 w-5" />}><div className="grid grid-cols-3 gap-3"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase text-slate-500">Height</p><p className="mt-1 font-bold text-[#0b2d54]">{text(computed.vitals?.heightCm, "—")} <span className="text-xs font-normal">cm</span></p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase text-slate-500">Weight</p><p className="mt-1 font-bold text-[#0b2d54]">{text(computed.vitals?.weightKg, "—")} <span className="text-xs font-normal">kg</span></p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase text-slate-500">BMI</p><p className="mt-1 font-bold text-[#0b2d54]">{text(computed.bmi, "—")}</p></div></div><Link href="/health-vitals" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">Open detailed vitals <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>
        </div>

        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <Panel title="Allergies" icon={<HeartPulse className="h-5 w-5" />}>{computed.allergies?.length ? <div className="space-y-2.5">{computed.allergies.slice(0, 5).map((item: any) => <div key={item.id} className="rounded-2xl bg-rose-50/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-[#0b2d54]">{item.name}</p><p className="mt-1 text-xs text-slate-600">Severity · {human(item.severity)}{item.reaction ? ` · Reaction · ${item.reaction}` : ""}</p></div><Badge tone={item.source === "PRACTITIONER" ? "rose" : "slate"}>{item.source === "PRACTITIONER" ? "Practitioner verified" : "Patient record"}</Badge></div></div>)}</div> : <Empty>No active allergies recorded.</Empty>}<Link href="/allergies" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">Manage allergies <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>

          <Panel title="Active conditions" icon={<Activity className="h-5 w-5" />}>{computed.conditions?.length ? <div className="space-y-2.5">{computed.conditions.slice(0, 5).map((item: any) => <div key={item.id} className="rounded-2xl bg-blue-50/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-[#0b2d54]">{item.name}</p><p className="mt-1 text-xs text-slate-600">{item.severity ? `Severity · ${human(item.severity)}` : ""}{item.chronic ? " · Chronic" : ""}{item.diagnosedAt ? ` · Recorded ${date(item.diagnosedAt)}` : ""}</p></div><Badge tone={item.source === "PRACTITIONER" ? "blue" : "slate"}>{item.source === "PRACTITIONER" ? "Clinical record" : "Patient record"}</Badge></div></div>)}</div> : <Empty>No active health conditions recorded.</Empty>}<Link href="/health-conditions" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">Manage conditions <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>
        </div>

        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <Panel title="Practitioner diagnoses" icon={<ShieldCheck className="h-5 w-5" />}>{computed.diagnoses?.length ? <div className="space-y-2.5">{computed.diagnoses.slice(0, 5).map((item: any) => <div key={item.id} className="rounded-2xl bg-[#f7f9fb] p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-[#0b2d54]">{item.name}</p><p className="mt-1 text-xs text-slate-600">{item.confirmed ? "Confirmed" : "Clinical record"}{item.severity ? ` · ${human(item.severity)}` : ""}{item.diagnosedAt ? ` · ${date(item.diagnosedAt)}` : ""}</p>{(item.practitionerName || item.diagnosedBy) && <p className="mt-1 text-xs text-slate-500">Recorded by · {text(item.practitionerName || item.diagnosedBy)}</p>}</div><Badge tone="blue">Read only</Badge></div></div>)}</div> : <Empty>No practitioner diagnoses are available yet.</Empty>}<Link href="/health-conditions" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">View clinical diagnoses <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>

          <Panel title="Procedures & surgical history" icon={<Activity className="h-5 w-5" />}>{computed.procedures?.length ? <div className="space-y-2.5">{computed.procedures.slice(0, 5).map((item: any) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-[#0b2d54]">{item.name}</p><p className="mt-1 text-xs text-slate-600">{item.surgical ? "Surgical" : "Procedure"}{item.performedAt ? ` · ${date(item.performedAt)}` : ""}{item.facility ? ` · ${item.facility}` : ""}</p>{item.outcome && <p className="mt-1 text-xs text-slate-500">Outcome · {item.outcome}</p>}</div><Badge>{item.status === "COMPLETED" ? "Completed" : human(item.status)}</Badge></div></div>)}</div> : <Empty>No procedures or surgical history recorded.</Empty>}</Panel>
        </div>

        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <Panel title="Current medications" icon={<HeartPulse className="h-5 w-5" />}>{computed.medications?.length ? <div className="space-y-2.5">{computed.medications.slice(0, 5).map((item: any) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><p className="font-bold text-[#0b2d54]">{item.name}</p><p className="mt-1 text-xs text-slate-600">{text(item.dosage, "Dose not recorded")}{item.frequency ? ` · ${human(item.frequency)}` : ""}{item.route ? ` · ${human(item.route)}` : ""}</p>{item.indication && <p className="mt-1 text-xs text-slate-500">For · {item.indication}</p>}</div>)}</div> : <Empty>No active medications recorded.</Empty>}<Link href="/medications" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">View medications <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>

          <Panel title="Immunizations" icon={<Syringe className="h-5 w-5" />}>{computed.immunizations?.length ? <div className="space-y-2.5">{computed.immunizations.slice(0, 5).map((item: any) => <div key={item.id} className="rounded-2xl bg-emerald-50/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-[#0b2d54]">{item.name}</p><p className="mt-1 text-xs text-slate-600">{item.administeredAt ? `Given ${date(item.administeredAt)}` : "Date not recorded"}{item.facility ? ` · ${item.facility}` : ""}</p>{item.nextDueDate && <p className="mt-1 text-xs font-semibold text-emerald-700">Next due · {date(item.nextDueDate)}</p>}</div><Badge tone={item.source === "PRACTITIONER" ? "green" : "slate"}>{item.source === "PRACTITIONER" ? "Clinician recorded" : "Patient record"}</Badge></div></div>)}</div> : <Empty>No immunizations recorded yet.</Empty>}<Link href="/immunizations" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">View immunizations <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>
        </div>

        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <Panel title="Emergency contacts" icon={<Phone className="h-5 w-5" />}>{computed.emergency?.contacts?.length ? <div className="space-y-2.5">{computed.emergency.contacts.slice(0, 2).map((item: any) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><p className="font-bold text-[#0b2d54]">{item.fullName}</p><p className="mt-1 text-xs text-slate-600">{human(item.relationship)} · {item.phoneNumber}</p></div>)}</div> : <Empty>No emergency contacts recorded.</Empty>}<Link href="/emergency-contacts" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">Manage contacts <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>

          <Panel title="Coverage" icon={<UserRound className="h-5 w-5" />}>{computed.coverage?.length ? <div className="space-y-2.5">{computed.coverage.slice(0, 2).map((item: any) => <div key={item.id} className="rounded-2xl bg-slate-50 p-4"><p className="font-bold text-[#0b2d54]">{item.providerName}</p><p className="mt-1 text-xs text-slate-600">{item.planName}</p><p className="mt-1 text-xs text-slate-500">Membership · {item.membershipNumber}</p></div>)}</div> : <Empty>No active coverage recorded.</Empty>}<Link href="/health-finance" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b2d54]">View coverage <ChevronRight className="h-3.5 w-3.5" /></Link></Panel>
        </div>

        <div className="mt-5 rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-[#0b2d54] text-white"><CalendarDays className="h-5 w-5" /></div><div><h2 className="font-bold text-[#0b2d54]">Your clinical record has provenance</h2><p className="mt-1 text-sm leading-6 text-slate-500">Patient-entered information stays editable by you. Practitioner diagnoses, verified allergies, clinical conditions and administered immunizations are shown in the same Clinic Card but remain clinically controlled.</p><p className="mt-2 text-xs text-slate-400">Last updated · {date(computed.lastUpdatedAt || computed.generatedAt)}</p></div></div></div>
      </div>
    </main>
  </ProtectedRoute>;
}
