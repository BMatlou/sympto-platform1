  ...
  return (
    <article className="flex h-full min-w-0 flex-col overflow-hidden rounded-[30px] border border-[#dfeaec] bg-white shadow-[0_18px_48px_rgba(11,45,84,.065)]">
      <div className="flex items-center justify-between gap-4 px-5 pb-4 pt-5 sm:px-6 sm:pt-6">
        ...
        <span className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-black ${targetReached ? "bg-[#eaf8ef] text-[#168660]" : onTrack ? "bg-[#e8f8f7] text-[#0b7b80]" : "bg-[#fff6e5] text-[#a26204]"}`}>
        ...
        {loading ? (
          ...
          <div className="overflow-hidden rounded-[27px] bg-[#0b2d54] text-white shadow-[0_16px_34px_rgba(11,45,84,.16)]">
            <div className="relative p-5 sm:p-6">
              ...
              <div className="relative mt-6 border-t border-white/10 pt-5">
                <div className="flex items-end justify-between gap-3">
                  <div><p className="text-[10px] font-black uppercase tracking-[.14em] text-white/45">Journey</p><p className="mt-1 text-sm font-bold text-white">{targetReached ? "Target reached" : `${formatKg(lostKg)} kg lost from ${formatKg(startingWeight)} kg`}</p></div>
            ...
            <div className="grid grid-cols-2 border-t border-white/10 sm:grid-cols-4">
              ...
              <div className="px-4 py-4"><p className="text-[9px] font-black uppercase tracking-[.12em] text-white/40">7-day change</p><p className="mt-1.5 text-sm font-black text-white">{weeklyChangeKg == null ? "Not enough history" : `${weeklyChangeKg > 0 ? "+" : ""}${formatKg(weeklyChangeKg)} kg`}</p></div>
            </div>
          </div>
        )}
      ...
      <div className={`${TODAY_GOAL_FOOTER_CLASS} bg-[#fbfdfd]`}>
        ...
        <p className="mt-2 text-[9px] leading-4 text-[#9aa8b1]">Progress is calculated from your recorded weight measurements.</p>
      </div>
    </article>
  );
}
