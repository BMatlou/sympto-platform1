...
function journeyFor(goal: any) {
  ...
  const totalDays = !Number.isNaN(startDate.getTime()) && !Number.isNaN(targetDate.getTime())
...
export default function TodayWeightGoal({ goal, fallbackWeight }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    healthGoalsService
      .getMetricEvents("WEIGHT", "weight.kg", from, new Date())
      .then((response) => {
        if (!active) return;
  ...
  const expectedProgress = journey.totalDays != null && journey.totalDays > 0
    ? Math.min(100, Math.max(0, ((journey.journeyDay - 1) / journey.totalDays) * 100))
    : null;
  ...
  const changeDirection = comparison === "INCREASE_TO" ? TrendingUp : TrendingDown;
  const ChangeIcon = changeDirection;

  return (
    <article className="mt-3.5 overflow-hidden rounded-[24px] border border-[#dfe9ed] bg-white shadow-[0_5px_18px_rgba(11,45,84,.035)]">
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#e5f7f6] text-[#0b6f73]"><Scale className="h-5 w-5" /></span>
            <div className="min-w-0">
              <p className="text-[9px] font-black uppercase tracking-[0.15em] text-[#0b6f73]">Weight goal</p>
              <h3 className="mt-1 truncate text-[17px] font-black tracking-[-.025em] text-[#0b2d54]">{String(goal?.title ?? "Reach your target weight")}</h3>
            </div>
          </div>
          ...
          <div className="mt-5 rounded-[18px] border border-[#e7eef1] bg-[#fbfdfd] p-4">
            <div className="flex items-end justify-between gap-4">
              ...
              <div className="text-right">
                <p className="text-[9px] font-black uppercase tracking-[0.12em] text-[#8a99a6]">Target</p>
                <p className="mt-1 text-sm font-black text-[#0b2d54]">{formatKg(targetWeight)} kg</p>
              </div>
            </div>

            <div className="mt-5 h-3 overflow-hidden rounded-full bg-[#edf2f5]">
              <div className="h-full rounded-full bg-gradient-to-r from-[#0b6f73] to-[#24c1c4] transition-[width] duration-500" style={{ width: `${progress}%` }} />
            </div>
            <div className="mt-2 flex items-center justify-between gap-3 text-[9px]">
              ...
              <span className="font-semibold text-[#8795a0]">{remainingKg === 0 ? "At target" : `${formatKg(remainingKg)} kg to go`}</span>
            </div>
      ...
      <div className="border-t border-[#edf2f5] px-4 py-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <p className="text-[9px] leading-4 text-[#8a99a6]">Progress is calculated from your recorded weight measurements.</p>
          <Link href="/health-goals" className="inline-flex min-h-10 shrink-0 items-center justify-center gap-1 rounded-xl border border-[#d7e4e8] bg-white px-3 py-2 text-[10px] font-black text-[#0b2d54]">View goal <ArrowRight className="h-3 w-3" /></Link>
        </div>
      </div>
    </article>
  );
