...
export default function TodayWeightGoal({ goal, fallbackWeight }: Props) {
  ...
  const targetReached = targetWeight != null && currentWeight != null && (comparison === "DECREASE_TO" ? currentWeight <= targetWeight : currentWeight >= targetWeight);
  ...
  const trendMessage = comparison === "DECREASE_TO"
    ? changeKg != null && changeKg > 0
  ...
  const statusLabel = currentBmiBelowRange
    ? "Gain weight focus"
    : bmiCaution
      ? "Goal needs review"
      : targetReached
  ...
  return (
    <article className="flex h-full min-w-0 flex-col overflow-hidden rounded-[30px] border border-[#dfeaec] bg-white shadow-[0_18px_48px_rgba(11,45,84,.065)]">
      <div className="flex items-center justify-between gap-4 px-5 pb-4 pt-5 sm:px-6 sm:pt-6">
        ...
        <span className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-black ${goalNeedsReview ? "bg-[#fff6e5] text-[#a26204]" : targetReached ? "bg-[#eaf8ef] text-[#168660]" : onTrack ? "bg-[#e8f8f7] text-[#0b7b80]" : "bg-[#fff6e5] text-[#a26204]"}`}>{statusLabel}</span>
      </div>
      ...
      <div className="flex-1 px-4 pb-5 sm:px-5 sm:pb-6">
        {loading ? (
          ...
          <div className="overflow-hidden rounded-[27px] bg-[#0b2d54] text-white shadow-[0_16px_34px_rgba(11,45,84,.16)]">
            <div className="relative p-5 sm:p-6">
              ...
              <div className="relative mt-6 border-t border-white/10 pt-5">
                <div className="flex items-end justify-between gap-3">
                  ...
                  {journey.daysLeft !== null && <p className="text-right text-[10px] font-bold text-white/50">{journey.daysLeft === 0 ? "Target date today" : `${journey.daysLeft} days left`}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 border-t border-white/10 sm:grid-cols-4">
              ...
              <div className="border-white/10 px-4 py-4 sm:border-r"><p className="text-[9px] font-black uppercase tracking-[.12em] text-white/40">Last recorded</p><p className="mt-1.5 text-sm font-black text-white">{formatDate(currentEvent?.occurredAt)}</p></div>
              <div className="px-4 py-4"><p className="text-[9px] font-black uppercase tracking-[.12em] text-white/40">BMI now</p><p className="mt-1.5 text-sm font-black text-white">{currentBmi == null ? "—" : currentBmi.toFixed(1)}</p></div>
            </div>
          </div>
        )}

        {!loading && currentWeight != null && !bmiCaution && targetAmount != null && remainingGoalAmount != null && (
        ...
        {!loading && currentWeight != null && (guidanceMessage || bmiCaution) && (
          <div className={`mt-4 rounded-[20px] border px-4 py-3.5 text-[10px] font-semibold leading-5 ${goalNeedsReview ? "border-amber-200 bg-amber-50 text-amber-900" : "border-[#dcebed] bg-[#f4fbfa] text-[#496a73]"}`}>
            <p className="font-black uppercase tracking-[.12em]">Weight &amp; BMI guidance</p>
      ...
      <div className={`${TODAY_GOAL_FOOTER_CLASS} bg-[#fbfdfd]`}>
        <div className="flex items-center justify-between gap-3 text-[10px] font-semibold text-[#74859a]">
          ...
          <Link href="/health-goals" className="inline-flex min-h-9 shrink-0 items-center justify-center gap-1.5 rounded-xl px-3 text-[10px] font-black text-[#0b2d54] hover:bg-white">View goal <ArrowRight className="h-3 w-3" /></Link>
        </div>
        <p className="mt-2 text-[9px] leading-4 text-[#9aa8b1]">Progress uses the weight recorded when this goal was created as the baseline, then applies later weight measurements to calculate actual change.</p>
      </div>
    </article>
  );
}
