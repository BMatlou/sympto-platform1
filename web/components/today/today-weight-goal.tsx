...
export default function TodayWeightGoal({ goal, fallbackWeight }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const goalBaseline = all.find((event) => event.source === "goal-baseline" && event.sourceId === String(goal?.id));
        const regular = all.filter((event) => !(event.source === "goal-baseline" && event.sourceId === String(goal?.id)));
        setBaselineWeight(goalBaseline?.loggedValue ?? null);
        setEvents(regular);
  ...
  const startingWeight = baselineWeight ?? currentWeight;
  const targetLossKg = comparison === "DECREASE_TO" ? configuredTarget : null;
  const targetGainKg = comparison === "INCREASE_TO" ? configuredTarget : null;
  const targetWeight = comparison === "DECREASE_TO" && startingWeight != null && targetLossKg != null ? startingWeight - targetLossKg : comparison === "INCREASE_TO" && startingWeight != null && targetGainKg != null ? startingWeight + targetGainKg : configuredTarget;
  ...
  const targetReached = achievedFromGoalStatus || (comparison === "DECREASE_TO" ? targetLossKg != null && lostKg != null && lostKg >= targetLossKg : targetGainKg != null && gainedKg != null && gainedKg >= targetGainKg);
  const progress = comparison === "DECREASE_TO" ? targetLossKg != null && lostKg != null && targetLossKg > 0 ? Math.round(Math.min(100, Math.max(0, (lostKg / targetLossKg) * 100))) : targetReached ? 100 : 0 : targetGainKg != null && gainedKg != null && targetGainKg > 0 ? Math.round(Math.min(100, Math.max(0, (gainedKg / targetGainKg) * 100))) : targetReached ? 100 : 0;
  ...
  const remainingGoalAmount = comparison === "DECREASE_TO" && targetLossKg != null && lostKg != null ? Math.max(targetLossKg - lostKg, 0) : comparison === "INCREASE_TO" && targetGainKg != null && gainedKg != null ? Math.max(targetGainKg - gainedKg, 0) : null;
  const requiredWeeklyChange = remainingGoalAmount != null && weeksLeft && weeksLeft > 0 ? remainingGoalAmount / weeksLeft : targetReached ? 0 : null;
