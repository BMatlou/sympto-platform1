...
export default function TodayWeightGoal({ goal, fallbackWeight }: Props) {
  ...
  const targetLabel = `${formatKg(remainingGoalAmount)} kg to target · target body weight ${formatKg(targetWeight)} kg`;
  ...
  return (
    <article className="flex h-full min-w-0 flex-col overflow-hidden rounded-[30px] border border-[#dfeaec] bg-white shadow-[0_18px_48px_rgba(11,45,84,.065)]">
      ...
      <div className="flex-1 px-4 pb-5 sm:px-5 sm:pb-6">
        ...
        {!loading && currentWeight != null && <div className={`mt-4 rounded-[20px] border px-4 py-3.5 text-[10px] font-semibold leading-5 ${changeKg != null && ((comparison === "DECREASE_TO" && changeKg > 0) || (comparison === "INCREASE_TO" && changeKg < 0)) ? "border-amber-200 bg-amber-50 text-amber-900" : "border-[#dcebed] bg-[#f4fbfa] text-[#496a73]"}`}>
          <p className="font-black uppercase tracking-[.12em]">Weight &amp; BMI trend</p>
          <p className="mt-1.5">{trendMessage}</p>
          {currentBmiBelowRange && <p className="mt-1.5">BMI now {currentBmi?.toFixed(1)} is below 18.5, so further weight loss should not be encouraged from BMI alone.</p>}
          {currentBmiAboveRange && <p className="mt-1.5">BMI now {currentBmi?.toFixed(1)} is at or above 25; interpret this screening measure with the rest of the person’s health information.</p>}
          {currentBmi == null && <p className="mt-1.5">BMI trend cannot be calculated until height is recorded.</p>}
          {bmiChange != null && Math.abs(bmiChange) >= 0.1 && <p className="mt-1.5">BMI change since goal start: {bmiChange > 0 ? "+" : ""}{bmiChange.toFixed(1)}.</p>}
          {bmiCaution && <p className="mt-1.5">The planned target BMI is {targetBmi?.toFixed(1)}, below 18.5. The target should be reviewed with a healthcare professional rather than treated as a required outcome.</p>}
        </div>}
      </div>
