    ...
    <section className="mt-3.5 overflow-hidden rounded-[28px] border border-[#dce9ee] bg-white shadow-[0_12px_34px_rgba(11,45,84,.055)]">
      <header className="flex items-center justify-between gap-3 border-b border-[#edf2f5] bg-gradient-to-br from-[#f9fdfd] via-white to-[#eef9f8] px-5 py-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#e5f7f6] text-[#0b6f73] ring-1 ring-[#d3efed]">
            <Pill className="h-5 w-5" />
          </span>
          <div className="min-w-0">
            <p className="text-[9px] font-black uppercase tracking-[.16em] text-[#0b7b80]">Prescribed medications</p>
            <h3 className="mt-0.5 text-base font-black tracking-[-.035em] text-[#0b2d54]">Medication today</h3>
          </div>
        </div>
        <span className="shrink-0 rounded-full bg-[#f1f6f8] px-2.5 py-1 text-[9px] font-black text-[#71839a]">
        ...
        <div className="p-4 sm:p-5">
          {prescriptions.map((medication, index) => {
            ...
            const isGoalSetForThisMed = activeGoals.some((goal) => {
              ...
              return (
                Boolean(recordId) &&
                goalMedicationId(goal) === recordId &&
            ...
            return (
              <article
                ...
                className="mb-4 last:mb-0 rounded-[24px] border border-[#e6eef1] bg-white p-5 shadow-[0_6px_20px_rgba(11,45,84,.035)]"
              >
                <div className="flex items-start gap-3">
                  <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#f1f6f8] text-[#0b2d54]">
                    <Pill className="h-4 w-4" />
                  </span>
                  <div className="min-w-0 flex-1">
                    <h4 className="text-sm font-black text-slate-900">{name}</h4>
                    <p className="mt-1 text-xs font-semibold uppercase tracking-[.04em] text-slate-500">{medicationSchedule(medication)}</p>
                    {doctor && <p className="mt-1 text-[10px] font-semibold text-[#80909e]">Prescribed by {doctor}</p>}
                  </div>
                </div>
                ...
                <div className="mt-4 border-t border-dashed border-[#dfe8ec] pt-3.5">
                  {isGoalSetForThisMed ? (
                    <div>
                      ...
                      <Link
                        href="#today-goals"
                        className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2.5 text-xs font-black text-emerald-700 transition hover:bg-emerald-100"
                      >
                    ...
                    <div>
                      <p className="mb-2 rounded-xl bg-amber-50/70 p-3 text-xs font-semibold leading-5 text-amber-800">
                        💡 New Prescription! You haven&apos;t set a tracking goal for this medication yet.
                      </p>
                      <Link
                        href="/health-goals"
                        className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 py-2.5 text-xs font-black text-white shadow-sm transition hover:bg-slate-800"
                      >
