    ...
    <ProtectedRoute>
      <main className="min-h-screen bg-[#f7f9fc]">
        <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          {loading ? <div className="mt-6 rounded-2xl bg-white p-6 text-sm text-slate-500">Loading family members…</div> : family.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-slate-200 bg-white p-10 text-center"><Users className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-4 font-semibold text-[#0b2d54]">No family members yet</p><p className="mt-1 text-sm text-slate-500">Add someone you are authorised to manage and choose what you can do on their behalf.</p></div> : <div className="mt-6 grid gap-4 sm:grid-cols-2">{family.map((member) => { const patientId = String(member.patientId ?? ""); const canOpen = Boolean(patientId) && Boolean(member.canViewRecords); return <div key={String(member.id)} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-slate-900">{member.name ? String(member.name) : "Name not available"}</p><p className="mt-1 text-sm text-slate-500">{member.relationship ? String(member.relationship) : "Relationship not recorded"}</p></div>{member.canReceiveAlerts && <CheckCircle2 className="h-5 w-5 text-[#24c1c4]" />}</div><div className="mt-4 flex flex-wrap gap-2 text-[11px] font-semibold">{member.canViewRecords && <span className="rounded-full bg-[#24c1c4]/10 px-2.5 py-1 text-[#0b2d54]">Can view records</span>}{member.canManageAppointments && <span className="rounded-full bg-slate-100 px-2.5 py-1 text-slate-600">Can manage appointments</span>}{member.canReceiveAlerts && <span className="rounded-full bg-slate-100 px-2.5 py-1 text-slate-600">Receives alerts</span>}</div>{canOpen ? <Link href={`/dashboard?patientId=${encodeURIComponent(patientId)}`} className="mt-5 inline-flex w-full items-center justify-center rounded-xl bg-[#0b2d54] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#071f3a]">Open health profile</Link> : <p className="mt-5 rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-500">Health records are not available through this family link.</p>}</div>; })}</div>}
        </div>
