        ...
        <div className="mx-auto flex min-h-[80vh] max-w-xl items-center justify-center">
          <section className="w-full rounded-[2rem] bg-white p-7 shadow-[0_20px_60px_rgba(11,45,84,0.10)] ring-1 ring-slate-200 sm:p-10">
            ...
            {result.actions.length > 0 && (
              <div className="mt-5 grid gap-2 sm:grid-cols-2">
                {result.actions.map((action) => (
                  <Link key={`${action.href}-${action.label}`} href={action.href} className={`flex min-h-14 items-center justify-center rounded-2xl border-2 px-4 text-center text-sm font-black transition focus:outline-none focus-visible:ring-4 focus-visible:ring-[#24c1c4]/20 ${urgent && action.href.startsWith("tel:") ? "border-red-200 bg-red-600 text-white hover:bg-red-700" : "border-slate-200 bg-white text-[#0b2d54] hover:border-[#24c1c4] hover:bg-slate-50"}`}>
                    {action.label}
                  </Link>
                ))}
              </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <Link href="/health-journal" className="flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-[#0b2d54] px-5 text-sm font-black text-white shadow-lg transition hover:bg-[#071f3a]"><Sparkles className="h-4 w-4" aria-hidden="true" /> View Smart Journal</Link>
              <Link href="/dashboard" className="flex min-h-14 items-center justify-center rounded-2xl border-2 border-slate-200 px-5 text-sm font-black text-[#0b2d54] transition hover:border-[#24c1c4]">Back to Health Home</Link>
            </div>
          </section>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-[#0b2d54] sm:px-6">
      <div className="mx-auto max-w-2xl">
        ...
        <section className="mt-5 rounded-[2rem] bg-white p-6 shadow-[0_20px_60px_rgba(11,45,84,0.10)] ring-1 ring-slate-200 sm:p-8">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#24c1c4]">Smart Journal</p><h1 className="mt-2 text-3xl font-black">How are you feeling?</h1><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Tell Sympto what you are experiencing. Your symptom becomes part of your connected health history so future entries can be compared with your other health information.</p></div><div className="hidden h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#0b2d54] text-white sm:flex"><ClipboardPlus className="h-6 w-6" aria-hidden="true" /></div></div>
          <label className="mt-8 block text-sm font-black text-[#0b2d54]" htmlFor="symptom">What are you feeling?</label>
          <input id="symptom" value={symptom} onChange={(e) => setSymptom(e.target.value)} placeholder="e.g. headache, tired, stomach pain" className="mt-2 min-h-14 w-full rounded-2xl border-2 border-slate-200 bg-white px-4 text-base font-semibold outline-none focus:border-[#24c1c4] focus:ring-4 focus:ring-[#24c1c4]/10" autoComplete="off" />
