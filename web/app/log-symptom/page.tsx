...
import { ArrowLeft, CheckCircle2, Mic, Save } from "lucide-react";
  ...
  const save = async () => {
    ...
    try {
      await healthJournalService.create({
        title: `Symptom — ${symptom.trim()}`,
        journal: `${symptom.trim()}\nSeverity: ${severity.toLowerCase()}\nStarted: ${started}.${details.trim() ? `\nDetails: ${details.trim()}` : ""}`,
        notes: "Patient-entered symptom update.",
      });
      setSaved(true);
    } catch {
      setError("We could not save your symptom. Please try again.");
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-8 text-[#0b2d54] sm:px-6">
        <div className="mx-auto flex min-h-[80vh] max-w-xl items-center justify-center">
          <section className="w-full rounded-[2rem] bg-white p-8 text-center shadow-[0_20px_60px_rgba(11,45,84,0.10)] ring-1 ring-slate-200 sm:p-12">
            <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#24c1c4]/15 text-[#0b2d54]">
              <CheckCircle2 className="h-10 w-10" aria-hidden="true" />
            </div>
            <p className="mt-6 text-xs font-black uppercase tracking-[0.18em] text-[#24c1c4]">Saved</p>
            <h1 className="mt-2 text-3xl font-black">Your symptom is in My Health Record</h1>
            <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-500">Keep tracking how you feel. Your future entries can help you and your care team see changes over time.</p>
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-[#0b2d54] sm:px-6">
      <div className="mx-auto max-w-2xl">
        <Link href="/" className="inline-flex min-h-12 items-center gap-2 rounded-xl px-2 text-sm font-bold text-slate-500 hover:bg-white hover:text-[#0b2d54]">
          <ArrowLeft className="h-5 w-5" aria-hidden="true" /> Back to Health Home
        </Link>
        ...
        <section className="mt-5 rounded-[2rem] bg-white p-6 shadow-[0_20px_60px_rgba(11,45,84,0.10)] ring-1 ring-slate-200 sm:p-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Tell Sympto what you are experiencing. Keep it simple — you can add more detail later.</p>
            </div>
            <div className="hidden h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#0b2d54] text-white sm:flex"><Mic className="h-6 w-6" /></div>
          </div>

          <label className="mt-8 block text-sm font-black text-[#0b2d54]" htmlFor="symptom">What are you feeling?</label>
          <div className="mt-2 flex gap-2">
            <input id="symptom" value={symptom} onChange={(e) => setSymptom(e.target.value)} placeholder="e.g. headache, tired, stomach pain" className="min-h-14 flex-1 rounded-2xl border-2 border-slate-200 bg-white px-4 text-base font-semibold outline-none focus:border-[#24c1c4] focus:ring-4 focus:ring-[#24c1c4]/10" />
          </div>
          ...
          <fieldset className="mt-7">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              {severityOptions.map((option) => {
                ...
                return <button key={option.value} type="button" onClick={() => setSeverity(option.value)} className={`min-h-20 rounded-2xl border-2 px-4 text-left transition ${selected ? "border-[#24c1c4] bg-[#24c1c4]/10" : "border-slate-200 bg-white hover:border-[#24c1c4]/50"}`} aria-pressed={selected}>
          ...
          <button type="button" onClick={save} disabled={!symptom.trim() || saving} className="mt-7 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#0b2d54] px-5 text-base font-black text-white shadow-lg transition hover:bg-[#071f3a] disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#24c1c4]/30">
