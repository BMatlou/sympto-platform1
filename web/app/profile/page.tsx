...
import { ArrowLeft, Camera, CheckCircle2, Edit3, MapPin, Settings, Users, ShieldAlert, X } from "lucide-react";
...
export default function ProfilePage() {
  ...
  async function load() {
    ...
    try {
      ...
      const addr = (dashboard.profile as ProfileData | null)?.address as ProfileData | null;
      const p = dashboard.patient ?? {};
      const hp = dashboard.healthPassport ?? {};
      const rawCountry = field(addr?.country as ProfileData | null, "iso2") || field(addr?.country as ProfileData | null, "name");
      const selectedCountry = countryOptions.find(c => c.code === rawCountry || c.name === rawCountry);
      ...
      setAddress({ addressLine1: field(addr, "line1"), addressLine2: field(addr, "line2"), suburb: field(addr, "suburb"), city: field(addr, "city"), province: field(addr, "province"), postalCode: field(addr, "postalCode"), country: selectedCountry?.code || rawCountry });
      setHealth({ dateOfBirth: field(person, "dateOfBirth").slice(0, 10) || field(p, "dateOfBirth").slice(0, 10), gender: field(person, "gender") || field(p, "gender") || "PREFER_NOT_TO_SAY", heightCm: field(p, "heightCm"), weightKg: field(p, "weightKg"), bloodType: field(hp, "bloodType") || field(p, "bloodType"), rhesusFactor: field(hp, "rhesusFactor") || field(p, "rhesusFactor"), dominantHand: field(p, "dominantHand"), occupation: field(p, "occupation"), smokingStatus: field(p, "smokingStatus"), alcoholConsumption: field(p, "alcoholConsumption"), exerciseFrequency: field(p, "exerciseFrequency") });
  ...
  async function saveAddress() {
    ...
    try {
      ...
      setEditing(null); setMessage("Address saved."); await load();
    }
  ...
  const addressText = [address.addressLine1, address.addressLine2, address.suburb, address.city, address.province, address.postalCode, address.country ? countryOptions.find(c => c.code === address.country)?.name || address.country : ""].filter(Boolean).join(", ");
  ...
  const stateOptions = address.country ? getStates(address.country) : [];
  const cityOptions = address.country && address.province ? getCities(address.country, address.province) : [];
  ...
  return <ProtectedRoute><main className="min-h-screen bg-slate-50"><div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    {loading ? <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-500">Loading your real profile data…</div> : <div className="mt-5 grid gap-5 lg:grid-cols-2">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold text-[#0b2d54]">Address</h2><p className="mt-1 text-sm text-slate-500">The primary address captured during registration. Country, province and city follow the same dependent selection logic used during sign-up.</p></div>{editing !== "address" && <button onClick={() => setEditing("address")} className="inline-flex items-center gap-2 rounded-xl bg-[#0b2d54] px-3 py-2 text-sm font-semibold text-white"><Edit3 className="h-4 w-4" />Edit</button>}</div>{editing === "address" ? <div className="mt-5 grid gap-4 sm:grid-cols-2">{([["Address line 1","addressLine1"],["Address line 2","addressLine2"],["Suburb","suburb"],["Postal code","postalCode"]] as [string,keyof AddressForm][]).map(([l,k]) => <div key={k}><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">{l}</label>{input(address[k], v => setAddress({...address,[k]:v}))}</div>)}<div><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">Country</label><select value={address.country} onChange={e => setAddress({...address, country:e.target.value, province:"", city:""})} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none focus:border-[#24c1c4]"><option value="">Select country</option>{countryOptions.map(c => <option key={c.code} value={c.code}>{c.name}</option>)}</select></div><div><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">Province / State</label><select value={address.province} disabled={!address.country} onChange={e => setAddress({...address, province:e.target.value, city:""})} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none disabled:bg-slate-50 disabled:text-slate-400 focus:border-[#24c1c4]"><option value="">{address.country ? "Select province / state" : "Select country first"}</option>{stateOptions.map(s => <option key={s.code} value={s.code}>{s.name}</option>)}</select></div><div><label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-400">City</label><select value={address.city} disabled={!address.province} onChange={e => setAddress({...address, city:e.target.value})} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none disabled:bg-slate-50 disabled:text-slate-400 focus:border-[#24c1c4]"><option value="">{address.province ? "Select city" : "Select province / state first"}</option>{cityOptions.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}</select></div><div className="flex gap-2 sm:col-span-2"><button disabled={saving} onClick={() => void saveAddress()} className="rounded-xl bg-[#24c1c4] px-4 py-2.5 text-sm font-bold text-[#0b2d54]">{saving ? "Saving…" : "Save changes"}</button><button onClick={() => setEditing(null)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold">Cancel</button></div></div> : <div className="mt-5 rounded-xl bg-slate-50 p-4"><MapPin className="mb-2 h-5 w-5 text-[#24c1c4]" /><p className="text-sm leading-6 text-slate-800">{addressText || "No address recorded yet. Add your address and it will be stored on your primary address record."}</p></div>}</section>
