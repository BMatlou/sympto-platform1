        ...
        <div className="mx-auto max-w-[1260px] px-4 pb-12 pt-4 sm:px-7 sm:pt-6">
          ...
          {(medicationGoal || smokingGoal || alcoholGoal || weightGoal) && <section className="mt-3.5 grid items-stretch gap-4 lg:grid-cols-2">
            ...
            {smokingGoal ? (() => {
              ...
              return <article className={`${TODAY_GOAL_CARD_CLASS} flex h-full min-w-0 flex-col`}>
                <div className="border-b border-[#edf2f5] bg-gradient-to-br from-[#f7fcfc] via-white to-[#eef8f8] px-5 py-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#e5f7f6] text-[#0b6f73]"><Cigarette className="h-4 w-4" /></span><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#0b6f73]">Smoking cessation tracker</p><h3 className="truncate text-lg font-black tracking-[-.04em] text-[#0b2d54]">{text(smokingGoal.title, "Smoking")}</h3></div></div>
                  ...
                  {todayLogged === null ? (
                    ...
                    <div className="rounded-[18px] border border-[#e7eef1] bg-[#fbfdfd] p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0"><p className="text-3xl font-black leading-none tracking-[-.06em] text-[#0b2d54]">{todayLogged}</p><p className="mt-1 text-sm font-bold text-[#74859a]">cigarettes today</p><p className="mt-2 text-[10px] font-semibold text-[#74859a]">Daily Target: {formatNumber(dailyTarget)} cigarettes</p></div>
                      ...
                      <div className="mt-5">
                        <div className="relative h-3 rounded-full bg-[#edf2f5]"><div className={`absolute inset-y-0 left-0 rounded-full ${exceeded ? "bg-[#d9775f]" : targetReached ? "bg-[#0b2d54]" : "bg-[#35b77a]"}`} style={{ width: `${todayBar}%` }} />{hasTarget && <span className="absolute -top-1.5 h-6 w-0.5 rounded-full bg-[#0b2d54]" style={{ left: `${targetMarker}%` }} aria-hidden="true" />}</div>
