...
const GOAL_META: Record<
  string,
  { label: string; icon: typeof Target; accent: string; surface: string }
> = {
...
export default function TodayPage() {
  ...
  const smokingGoal = goals.find(
    (goal: any) => String(goal?.category ?? "").toUpperCase() === "SMOKING",
  );
  const medicationGoal = goals.find(
    (goal: any) => String(goal?.category ?? "").toUpperCase() === "MEDICATION",
  );
  const otherGoals = goals.filter(
    (goal: any) => !["SMOKING", "MEDICATION"].includes(String(goal?.category ?? "").toUpperCase()),
  );
    ...
    void healthGoalsService
      ...
      .then((response) => {
        ...
        const event = response.events.find(
          (item) => item.sourceId === `${String(smokingGoal.id)}:${dayKey}`,
        );
        ...
        <main className="min-h-screen bg-[#f5fafb] p-4 sm:p-8">
          <div className="mx-auto max-w-xl rounded-[28px] border border-red-200 bg-white p-7">
            ...
            <button
              type="button"
              onClick={reload}
              className="mt-5 min-h-11 rounded-xl bg-[#0b2d54] px-5 py-2.5 text-sm font-semibold text-white"
            >
              Try again
            </button>
          </div>
  ...
  const todayAppointments = appointments.filter(
    (appointment: any) =>
      ...
      new Date(String(appointment.scheduledStart)).toDateString() === new Date().toDateString(),
  );

  const actionItems = [
    ...attention.slice(0, 3).map((item: any) => ({
      href: String(item.actionUrl || "/health-journal"),
      label: "Needs review",
      title: text(item.title, "Attention needed"),
    ...
    ...todayAppointments.slice(0, 2).map((appointment: any) => ({
      href: "/appointments",
      label: "Appointment",
      title: text(appointment.reason || appointment.appointmentType, "Clinic visit").replaceAll("_", " "),
      detail: formatDate(appointment.scheduledStart, true),
      icon: CalendarDays,
    })),
    ...careTasks.slice(0, 2).map((task: any) => ({
      href: "/care-plans",
      label: "Care plan",
      title: text(task.title, "Care task"),
      detail: task.dueDate ? `Due ${formatDate(task.dueDate)}` : text(task.carePlanTitle, "Care plan"),
      icon: ClipboardCheck,
    })),
    ...notifications
      ...
      .slice(0, 2)
      .map((item: any) => ({
        href: String(item.actionUrl || "/today"),
        label: "Reminder",
        title: text(item.title, "Reminder"),
        detail: text(item.body),
        icon: Bell,
      })),
  ];
  ...
  const smokingTargetPosition =
    todaySmoking !== null && todaySmoking > 0 && smokingTarget > 0
      ...
      <main className="min-h-screen bg-[#f5fafb] text-[#17314e]">
        <div className="mx-auto max-w-[1260px] px-4 pb-12 pt-4 sm:px-7 sm:pt-6">
          <header className="mb-5 flex items-center justify-between">
            <Link
              href="/dashboard"
              className="inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-sm font-semibold text-[#0b2d54] hover:bg-white"
            >
            ...
            <div className="mt-3 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-xl text-sm leading-6 text-white/70">
                  Your important health actions, brought together in one place.
                </p>
              </div>
              <Link
                href="/log-symptom"
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-white/12 px-4 py-2 text-xs font-bold text-white ring-1 ring-white/20"
              >
              ...
              <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[11px] text-[#74859a]">
                      {text(medications[0]?.dosage, "Dose not recorded")} · {String(medications[0]?.frequency || "Schedule not recorded").replaceAll("_", " ")}
                    </p>
                  </div>
                </div>
                <a href="#today-goals" className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-[#0b2d54] px-4 py-2.5 text-[10px] font-black text-white">
                ...
                <article className="flex h-full flex-col overflow-hidden rounded-[24px] border border-[#dfe9ed] bg-white shadow-[0_5px_18px_rgba(11,45,84,.035)]">
                  <div className="flex-1 p-5 sm:p-6">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {todaySmoking !== null && <span className={`shrink-0 rounded-full px-2.5 py-1 text-[9px] font-black ${smokingExceeded ? "bg-red-50 text-red-700" : "bg-[#e9f9fa] text-[#0b6f73]"}`}>{smokingExceeded ? `${smokingDifference} over` : "On target"}</span>}
                    </div>

                    <div className="mt-5 flex items-end justify-between gap-4">
                      <div>
                        <p className="text-3xl font-black tracking-[-.06em] text-[#0b2d54]">{todaySmoking === null ? "—" : todaySmoking}</p>
                        <p className="mt-1 text-[10px] text-[#74859a]">today · target {numberText(smokingTarget)}</p>
                      </div>
                      {smokingExceeded && <p className="max-w-[150px] text-right text-[10px] font-semibold leading-4 text-[#842832]">{smokingDifference} above target. Every next choice counts.</p>}
                    </div>

                    <div className="mt-5">
                      <div className="mb-2 flex items-center justify-between gap-2 text-[9px] font-black uppercase tracking-[0.12em] text-[#74859a]">
                        <span>Today&apos;s target</span>
                        <span className={smokingExceeded ? "text-[#de6c5f]" : "text-[#0b6f73]"}>{todaySmoking === null ? "Not logged" : smokingExceeded ? `${smokingDifference} above target` : "On target"}</span>
                      </div>
                      ...
                      <div className="relative h-[62px] overflow-visible rounded-2xl border border-[#dfebef] bg-gradient-to-b from-[#f8fbfc] to-[#edf4f6] px-3">
                        <div className="absolute inset-x-3 top-1/2 flex h-6 -translate-y-1/2 items-stretch drop-shadow-[0_4px_6px_rgba(11,45,84,.12)]">
                          ...
                          <div className="relative flex-1 rounded-r-md border border-l-0 border-[#cedddd] bg-gradient-to-b from-white via-[#f0f4f2] to-[#d8e3e2]">
                            <span className="absolute right-[18px] top-[-1px] bottom-[-1px] w-[5px] bg-gradient-to-r from-[#b95d36] via-[#f19b55] to-[#793e31] shadow-[0_0_8px_rgba(238,117,45,.5)]" />
                          </div>
                          <div className="relative h-6 w-[22px] rounded-r-lg bg-[radial-gradient(circle_at_25%_50%,#fff7a7_0_8%,#ffcb55_18%,#ef6b2d_48%,#8f3e32_80%)] shadow-[0_0_5px_#ffb02e,0_0_12px_rgba(255,94,35,.85)] animate-pulse">
                            <span className="absolute right-1 top-[-10px] h-3 w-2 rotate-[25deg] rounded-full bg-gradient-to-b from-[#fff09c] to-[#ff8a32]" />
                            <span className="absolute right-1 top-[-18px] h-5 w-2 border-l-2 border-slate-400/40 rounded-full opacity-60" />
                          </div>
                        </div>
                      ...
                      <div className="mt-1.5 flex items-center justify-between text-[9px] font-semibold text-[#8795a0]">
                        <span>0</span>
                        <span className="font-black text-[#0b2d54]">Target {numberText(smokingTarget)}</span>
                        <span className={smokingExceeded ? "font-black text-[#de6c5f]" : "font-black text-[#0b6f73]"}>Today {todaySmoking ?? "—"}</span>
                      </div>
                    </div>
              ...
              {otherGoals.map((goal: any) => {
                ...
                return (
                  <article key={String(goal.id)} className="rounded-[24px] border border-[#e0ebee] bg-white p-5 shadow-[0_5px_18px_rgba(11,45,84,.03)]">
                    <div className="flex items-center gap-3">
                      <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${meta.surface} ${meta.accent}`}><Icon className="h-4 w-4" /></span>
          ...
          {!medicationGoal && !smokingGoal && (
            <Link href="/health-goals" className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-xl bg-[#0b2d54] px-4 py-2.5 text-[10px] font-black text-white">
              Set a health goal <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          )}
        </div>
      </main>
