...
function goalTarget(goal: any) {
  ...
  if (number === null) return "Target not set";
...
export default function TodayPage() {
  ...
  useEffect(() => {
    const smokingGoals = (data?.goals ?? []).filter((goal: any) => String(goal?.status).toUpperCase() === "ACTIVE" && String(goal?.category ?? "").toUpperCase() === "SMOKING");
    ...
    void healthGoalsService.getMetricEvents("SMOKING", "smoking.cigarettes", start, end, "patient-smoking-log")
      .then((response) => {
        if (cancelled) return;
        const next: Record<string, number | null> = {};
        const drafts: Record<string, string> = {};
        for (const goal of smokingGoals) {
          const sourceId = `${String(goal.id)}:${dayKey}`;
          const event = response.events.find((item) => item.sourceId === sourceId);
      ...
      .catch(() => {
        // The card remains usable; absence of the daily event is treated as "not logged".
      });
  ...
  return <ProtectedRoute><main className="min-h-screen bg-[#f5fafb] text-[#17314e]"><div className="mx-auto max-w-[1260px] px-4 pb-12 pt-4 sm:px-7 sm:pt-6">
    ...
    {displayGoals.length > 0 && <section className="mt-4 overflow-hidden rounded-[30px] border border-[#dce9ee] bg-white shadow-[0_18px_48px_rgba(11,45,84,.055)]"><div className="grid gap-4 p-4 sm:grid-cols-2 sm:p-5 xl:grid-cols-3">{displayGoals.map((goal: any) => {
      ...
      return <article key={String(goal.id)} className="group relative overflow-hidden rounded-[25px] border border-[#e0ebee] bg-gradient-to-br from-white via-white to-[#f7fbfc] p-5 transition duration-200 hover:-translate-y-0.5 hover:border-[#c8dce1] hover:shadow-[0_18px_38px_rgba(11,45,84,.09)]">
        ...
        <div className="relative flex items-start justify-between gap-3"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${meta.surface} ${meta.accent}`}><Icon className="h-5 w-5" /></span><span className={`rounded-full px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.12em] ${isSmoking ? smokingStatus.tone : status.tone}`}>{isSmoking ? smokingStatus.label : status.label}</span></div>
        <div className="relative mt-5"><p className={`text-[10px] font-black uppercase tracking-[0.15em] ${meta.accent}`}>{meta.label}</p><h4 className="mt-1.5 truncate text-[17px] font-black tracking-[-.025em] text-[#0b2d54]">{text(goal.title, "Health goal")}</h4><p className="mt-2 text-[11px] leading-5 text-[#74859a]">{isSmoking ? (todaySmoking === null ? `Today’s target · ${formatNumber(smokingTarget)} cigarettes/day` : `${formatNumber(todaySmoking)} cigarettes today · ${formatNumber(smokingTarget)} cigarettes/day target`) : goalInsight(goal, progress)}</p></div>

        {isSmoking && smokingExceeded ? <div className="relative mt-5 overflow-hidden rounded-[22px] border border-red-200 bg-white shadow-[0_10px_26px_rgba(190,24,93,.05)]">
          <div className="border-b border-red-100 bg-gradient-to-r from-red-50 via-rose-50 to-white px-4 py-4 sm:px-5"><p className="text-[13px] font-black leading-6 text-red-800 sm:text-sm">❗ You are {differenceDelta} cigarettes above today’s target. Every next choice still counts.</p></div>
          <div className="grid gap-3 p-4 sm:grid-cols-3 sm:p-5">
            <div className="rounded-2xl border border-[#e7eff1] bg-[#f8fbfc] p-4"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#97a7b4]">Today</p><p className="mt-1.5 text-sm font-black text-[#0b2d54]">Today: {todaySmoking} cigarettes</p></div>
            <div className="rounded-2xl border border-[#e7eff1] bg-[#f8fbfc] p-4"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#97a7b4]">Daily target</p><p className="mt-1.5 text-sm font-black text-[#0b2d54]">Daily target: {dailyTarget} cigarettes</p></div>
            <div className="rounded-2xl border border-red-100 bg-red-50/70 p-4"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-red-500">Today&apos;s difference</p><p className="mt-1.5 text-sm font-black text-red-700">Today&apos;s difference: +{differenceDelta} above target</p></div>
          </div>
          <div className="px-4 sm:px-5"><div className="rounded-2xl border border-[#e7eff1] bg-white p-4"><p className="text-lg font-black tracking-[-.03em] text-[#0b2d54]">{compliancePercentage}% of daily target</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#eef2f4]"><div className="h-full rounded-full bg-red-400 transition-all duration-300" style={{ width: `${Math.min(100, compliancePercentage)}%` }} /></div></div></div>
          <div className="mt-4 border-t border-[#edf2f5] px-4 py-4 sm:px-5"><p className="text-xs font-black text-[#0b2d54]">Journey timeline: Your program is moving one day at a time.</p><p className="mt-1 text-[11px] font-semibold text-[#74859a]">Day {smokingTimeline?.currentDayOfJourney ?? 1} · {smokingTimeline?.daysLeft ?? 0} days left · Today ({formatDate(smokingTimeline?.targetDate ?? goal?.targetDate ?? "2026-09-30T00:00:00Z", false)} Target date)</p><p className="mt-1 text-[11px] font-semibold text-[#74859a]">{todaySmoking} cigarettes logged today</p></div>
          <div className="flex flex-col gap-2.5 border-t border-[#edf2f5] p-4 sm:flex-row sm:p-5">
            <button type="button" onClick={() => { setOpenSmokingGoalId(String(goal.id)); setSmokingDrafts((currentDrafts) => ({ ...currentDrafts, [String(goal.id)]: String(todaySmoking) })); }} className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl bg-[#0b2d54] px-4 py-2.5 text-[10px] font-black text-white shadow-sm transition hover:bg-[#123d63]">Update today&apos;s log</button>
            <Link href="/health-goals" className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl border border-[#d7e4e8] bg-white px-4 py-2.5 text-[10px] font-black text-[#0b2d54] transition hover:bg-[#f7fbfc]">View goal</Link>
          </div>
          {openSmokingGoalId === String(goal.id) && <div className="border-t border-[#edf2f5] bg-[#f5fafb] p-4 sm:p-5"><label className="text-[9px] font-black uppercase tracking-[0.14em] text-[#74859a]" htmlFor={`smoking-${goal.id}`}>Cigarettes smoked today</label><div className="mt-2 flex gap-2"><input id={`smoking-${goal.id}`} type="number" min="0" step="1" inputMode="numeric" value={smokingDrafts[String(goal.id)] ?? ""} onChange={(event) => setSmokingDrafts((currentDrafts) => ({ ...currentDrafts, [String(goal.id)]: event.target.value }))} placeholder="0" className="min-h-10 min-w-0 flex-1 rounded-xl border border-[#d7e4e8] bg-white px-3 text-sm font-bold text-[#0b2d54] outline-none focus:border-[#24c1c4] focus:ring-2 focus:ring-[#24c1c4]/15" /><button type="button" disabled={savingSmokingGoalId === String(goal.id)} onClick={() => void saveSmokingToday(goal)} className="min-h-10 rounded-xl bg-[#24c1c4] px-4 text-[10px] font-black text-white disabled:opacity-50">{savingSmokingGoalId === String(goal.id) ? "Saving…" : "Save"}</button></div><button type="button" onClick={() => setOpenSmokingGoalId(null)} className="mt-2 text-[10px] font-bold text-[#74859a]">Cancel</button></div>}
        </div> : isSmoking ? <div className="relative mt-5 rounded-[19px] border border-[#e7eff1] bg-white/90 p-4">
        ...
        {!smokingExceeded && <div className="relative mt-3 flex items-center justify-between gap-3 text-[10px] font-semibold text-[#74859a]"><span>{isSmoking ? (todaySmoking === null ? "Daily check-in needed" : `Today ${formatNumber(todaySmoking)} cigarettes`) : current ? `Current ${current}` : goalFrequency(goal)}</span><span className="shrink-0">{goal?.targetDate ? formatDate(goal.targetDate, false) : "No date"}</span></div>}
        {!smokingExceeded && <div className="relative mt-4 flex items-center justify-between border-t border-[#edf2f5] pt-3"><Link href="/health-goals" className="text-[10px] font-black text-[#0b2d54] hover:underline">View goal</Link><ArrowRight className="h-3.5 w-3.5 text-[#91a0ad]" /></div>}
      </article>;
    })}</div></section>}
