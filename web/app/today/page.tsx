              ...
              return <article className="flex h-full flex-col rounded-[24px] border border-[#dfe9ed] bg-white shadow-[0_5px_18px_rgba(11,45,84,.035)]">
                ...
                <div className="border-t border-[#edf2f5] px-4 py-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" disabled={loggingLocked} aria-disabled={loggingLocked} onClick={() => { if (loggingLocked) return; setOpenSmokingGoalId(goalId); setSmokingDrafts((current) => ({ ...current, [goalId]: todayLogged === null ? "" : String(todayLogged) })); }} className={`min-h-10 rounded-xl px-3 py-2 text-[10px] font-black transition ${loggingLocked ? "cursor-not-allowed bg-[#eef2f4] text-[#93a0aa]" : "bg-[#0b2d54] text-white hover:bg-[#123e66]"}`}>{savingSmokingGoalId === goalId ? "Saving…" : targetReached ? "Daily target reached" : todayLogged === null ? "Log today’s smoking" : "Update today’s log"}</button>
