...
function goalAction(goal: any, medications: any[]) {
  const category = String(goal?.category ?? "").toUpperCase();
  if (category === "MEDICATION") {
    const names = medications.slice(0, 2).map((item) => item?.medication?.name || item?.medication?.genericName || item?.name).filter(Boolean);
    const label = names.length ? names.join(" · ") : "your scheduled medicines";
    return {
      title: "Keep your medication plan on track",
      detail: `Today: mark each dose Taken or Skipped for ${label}.`,
      href: "/medications",
      cta: "Update medication",
    };
  }
  if (category === "EXERCISE") return { title: "Complete your exercise activity", detail: "Record today’s exercise in the check-in to move this goal forward.", href: "/today", cta: "Log exercise" };
  if (category === "HYDRATION") return { title: "Keep drinking toward your water target", detail: "Add water in the Daily Health Check-in as you go.", href: "/today", cta: "Add water" };
  if (category === "SLEEP") return { title: "Record last night’s sleep", detail: "Add your sleep hours in the check-in when sleep tracking is enabled.", href: "/today", cta: "Log sleep" };
  return { title: "Keep working on this goal", detail: "Use the activity that matches this goal to build progress over time.", href: "/health-goals", cta: "View goal" };
}
...
export default function TodayPage() {
  ...
  const featuredGoalAction = featuredGoal ? goalAction(featuredGoal, medications) : null;
  ...
  return <ProtectedRoute><main className="min-h-screen bg-[#f5fafb] text-[#17314e]"><div className="mx-auto max-w-[1260px] px-4 pb-12 pt-4 sm:px-7 sm:pt-6">
    ...
    <section className="mt-4 rounded-[27px] border border-[#dfebef] bg-white p-[22px] shadow-[0_6px_20px_rgba(11,45,84,.035)]"><div className="flex items-start justify-between gap-4"><div><h3 className="text-[17px] font-black text-[#0b2d54]">Health goals</h3><p className="mt-2 text-xs leading-5 text-[#74859a]">Keep your longer-term goals moving.</p></div><Link href="/health-goals" className="rounded-[11px] bg-[#edf4ff] px-2.5 py-2 text-[10px] font-black text-[#3f75bd]">{goals.length} active goal{goals.length === 1 ? "" : "s"}</Link></div>{featuredGoal && featuredGoalAction ? <div className="mt-4 border-t border-[#eef3f4] pt-4"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf4ff] text-[#3f75bd]"><Target className="h-4 w-4" /></span><div><b className="block text-sm text-[#0b2d54]">{text(featuredGoal.title)}</b><span className="mt-0.5 block text-[11px] text-[#74859a]">{goalProgress(featuredGoal)}% progress</span></div></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#edf2f5]"><div className="h-full rounded-full bg-[#24c1c4] transition-all" style={{ width: `${goalProgress(featuredGoal)}%` }} /></div><p className="mt-3 text-sm font-bold text-[#0b2d54]">{featuredGoalAction.title}</p><p className="mt-1 text-xs leading-5 text-[#74859a]">{featuredGoalAction.detail}</p></div><Link href={featuredGoalAction.href} className="shrink-0 rounded-xl bg-[#0b2d54] px-3.5 py-2.5 text-[10px] font-black text-white">{featuredGoalAction.cta} →</Link></div></div> : <div className="mt-4 border-t border-[#eef3f4] pt-4"><p className="text-sm text-[#74859a]">Create a health goal to start tracking meaningful progress.</p></div>}<Link href="/health-goals" className="mt-4 inline-block text-xs font-black text-[#0b2d54]">Open health goals →</Link></section>
