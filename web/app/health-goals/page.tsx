...
function weightAdvice(weight: number | null, height: number | null, amount: number | null, direction: "LOSE" | "GAIN", targetDate: string) {
  if (weight == null || amount == null || amount <= 0) return null;
  const bmi = bmiFor(weight, height);
  const targetWeight = direction === "LOSE" ? weight - amount : weight + amount;
  ...
  const weekly = weeks ? amount / weeks : null;
...
export default function HealthGoalsPage() {
  ...
  const reviseLossAmount = advisor?.lowerHealthyWeight != null && Number.isFinite(currentWeight) ? Math.max(0, currentWeight - advisor.lowerHealthyWeight) : null;
  const canReviseLossTarget = Boolean(advisor?.targetBelowHealthy && draft.weightDirection === "LOSE" && reviseLossAmount != null && reviseLossAmount > 0);
  ...
  useEffect(() => {
    ...
    setDraft({ title: String(goal.title ?? ""), description: String(goal.description ?? ""), category: category.value, priority: String(goal.priority ?? "MEDIUM"), targetValue: goal.targetValue == null ? "" : String(goal.targetValue), unit: String(goal.unit ?? category.unit), targetDate: goal.targetDate ? String(goal.targetDate).slice(0, 10) : "", weightDirection: String(goal?.metricConfig?.comparison ?? "DECREASE_TO").toUpperCase() === "INCREASE_TO" ? "GAIN" : "LOSE" });
    setEditorOpen(true);
  ...
  function chooseCategory(category: string) { const config = CATEGORIES.find((item) => item.value === category); setDraft((current) => ({ ...current, category, unit: config?.unit ?? "", title: current.title || config?.label || "", weightDirection: category === "WEIGHT" ? current.weightDirection : "LOSE" })); }
  ...
  async function saveGoal() {
    ...
    try {
      setSaving(true);
      const comparison = draft.category === "WEIGHT" ? draft.weightDirection === "GAIN" ? "INCREASE_TO" : "DECREASE_TO" : config.comparison;
