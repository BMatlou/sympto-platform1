...
import { ArrowLeft, Droplets, HeartPulse, ShieldCheck, Syringe, UserRound } from "lucide-react";
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[#24c1c4]/10 px-3 py-1 text-xs font-semibold text-[#0b2d54]">
            <ShieldCheck className="h-3.5 w-3.5" />My health
          </div>
          ...
          <>
            ...
            <section className="mb-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-white p-6"><Droplets className="h-5 w-5 text-[#24c1c4]" /><p className="mt-4 text-xs font-medium text-slate-400">Blood type</p><p className="mt-1 text-3xl font-bold text-[#0b2d54]">{formatEnum(healthPassport?.bloodType)}</p></div>
                <div className="rounded-2xl border border-slate-200 bg-white p-6"><HeartPulse className="h-5 w-5 text-[#24c1c4]" /><p className="mt-4 text-xs font-medium text-slate-400">Rhesus factor</p><p className="mt-1 text-3xl font-bold text-[#0b2d54]">{formatEnum(healthPassport?.rhesusFactor)}</p></div>
              </div>
            </section>

            <section className="mb-6">
              <h2 className="mb-4 text-lg font-semibold text-[#0b2d54]">Important information</h2>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <InfoCard label="Allergies" value={String(activeAllergies.length)} description={activeAllergies.length ? "Active allergies recorded" : "No active allergies recorded"} />
                <InfoCard label="Health conditions" value={String(activeConditions.length)} description={activeConditions.length ? "Active conditions recorded" : "No active conditions recorded"} />
                <InfoCard label="Immunizations" value={String(immunizations.length)} description={immunizations.length ? "Immunizations recorded" : "No immunizations recorded"} />
                <InfoCard label="Emergency contacts" value={String(emergencyContacts.length)} description="Contacts available in an emergency" />
              </div>
            </section>
            ...
            <section className="mb-6">
              <div className="mb-4 flex items-center gap-2"><Syringe className="h-5 w-5 text-[#24c1c4]" /><h2 className="text-lg font-semibold text-[#0b2d54]">Immunizations</h2></div>
              <div className="rounded-2xl border border-slate-200 bg-white p-6">
                {immunizations.length ? (
                  <div className="grid gap-3 sm:grid-cols-2">
                    {immunizations.map((item: any) => (
                      <div key={item?.id ?? `${item?.immunizationId}-${item?.doseNumber}`} className="rounded-xl bg-slate-50 p-4">
                        <p className="font-medium text-[#0b2d54]">{item?.immunization?.name || item?.name || "Immunization"}</p>
                        {item?.administeredAt && <p className="mt-1 text-sm text-slate-500">Administered: {new Date(item.administeredAt).toLocaleDateString("en-ZA")}</p>}
                        {item?.doseNumber != null && <p className="mt-1 text-sm text-slate-500">Dose {item.doseNumber}</p>}
                        {item?.facility && <p className="mt-1 text-sm text-slate-500">Facility: {item.facility}</p>}
                      </div>
                    ))}
                  </div>
                ) : <div className="flex items-center gap-3"><Syringe className="h-5 w-5 text-slate-400" /><p className="text-sm text-slate-500">No immunizations are currently recorded.</p></div>}
              </div>
            </section>

            <div className="mt-8 rounded-2xl border border-[#24c1c4]/20 bg-[#24c1c4]/5 p-5"><div className="flex gap-3"><ShieldCheck className="h-5 w-5 shrink-0 text-[#0b2d54]" /><div><h3 className="font-semibold text-[#0b2d54]">Your health passport is private</h3><p className="mt-1 text-sm leading-6 text-slate-500">Keep this information accurate and up to date.</p></div></div></div>
          </>
        )}
      </div>
    </main>
