...
function Field({ label, name, value, onChange, placeholder, step = "1", min, max }: { label: string; name: string; value: string; onChange: (value: string) => void; placeholder: string; step?: string; min?: string; max?: string }) {
...
export default function HealthVitalsPage() {
  ...
  async function saveManualVitals(event: FormEvent) {
    ...
    for (const key of Object.keys(mappings)) {
      const raw = manual[mappings[key]];
      const value = Number(raw);
      if (raw !== "" && Number.isFinite(value)) input[key] = value;
    }
    if (Object.keys(input).length === 0) { setMessage("Enter at least one vital before saving."); return; }

    const temperature = input.bodyTemperature;
    if (temperature !== undefined && (temperature < 30 || temperature > 45)) {
      setMessage("Enter body temperature in °C. A value from 30.0°C to 45.0°C is supported; for example, 36.7°C.");
      return;
    }
