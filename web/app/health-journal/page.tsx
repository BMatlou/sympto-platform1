...
export default function HealthJournalPage() {
  ...
  const counts = { visits: (data.encounters ?? []).length, symptoms: (data.symptoms ?? []).length, labs: (data.recentResults?.laboratory ?? []).length, imaging: (data.recentResults?.imaging ?? []).length, prescriptions: (data.prescriptions ?? []).length, documents: (data.attachments ?? []).length };
  ...
  return <ProtectedRoute><main className="min-h-screen bg-[#f5f8fb]"><div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
    ...
    <section className="mb-5 rounded-[30px] bg-gradient-to-br from-blue-700 via-blue-600 to-[#0b2d54] p-6 text-white shadow-lg sm:p-8"><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10"><Sparkles className="h-6 w-6"/></div><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-white/65">My History & Files</p><h1 className="mt-1 text-3xl font-bold">Your health story</h1></div></div><p className="mt-4 max-w-2xl text-sm leading-6 text-white/75">Visits, symptoms, results, prescriptions and documents are brought together automatically from your health record.</p><div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-6">{Object.entries(counts).map(([key, value]) => <div key={key} className="rounded-xl bg-white/10 p-3"><p className="text-[10px] uppercase text-white/55">{key}</p><p className="mt-1 text-lg font-bold">{value}</p></div>)}</div></section>
