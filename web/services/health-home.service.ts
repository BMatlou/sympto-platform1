...
export interface HealthHomeResponse {
  ...
  healthSnapshot: { activeConditions: Array<Record<string, unknown>>; allergies: Array<Record<string, unknown>>; bloodType?: string | null; rhesusFactor?: string | null; baseline?: Record<string, unknown> | null; latestMeasurements: Array<Record<string, unknown>>; connectedDevices: Array<Record<string, unknown>> };
  today: { upcomingAppointments: Array<Record<string, unknown>>; activeMedications: Array<Record<string, unknown>>; activeMedicationCount: number; activeGoalCount: number };
  ...
  attention: Array<{ type: string; severity: string; title: string; description: string; actionUrl?: string | null }>;
  ...
  async getHealthHome(): Promise<HealthHomeResponse> {
    ...
    return {
      ...
      wearables: data.wearables ?? {
        ...
        latestMeasurements: data.healthSnapshot.latestMeasurements.map((m: any, index) => ({ id: `${m.type}-${index}`, type: m.type, value: m.value, unit: m.unit, measuredAt: m.measuredAt, source: m.source })),
      },
    };
  }
}
